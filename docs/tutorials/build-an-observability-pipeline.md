...
Emit three events (DEBUG/INFO/WARN): the file sink (floor DEBUG) keeps all three; the console sink (floor WARN) keeps one — the *same* fanned event, independent decisions (`logs/main.rs:234-253`). `LevelGate` is named "Gate" but is really a filter: it decides per record (level ≥ threshold) and forwards only survivors — not the armed/disarmed readiness switch Foundations calls `gate`. There is no `Filter` type to reach for; a filter in proxima is a **predicate that is itself a pipe**, and `PipeExt::filter(pred)` is literally `AndThen::new(predicate, self)` (`proxima-primitives/src/pipe/ext.rs:57–62`). `LevelGate` writes that decision pipe out by hand for one reason: the decision pipes shipped in `filter.rs` compose over `Request<Bytes>`, and this pipeline's payload is a `TelemetryRequest` — the same reason `gate.rs`'s `Gated<G>` composes its own (`logs/main.rs:98-108`). See [`examples/fan_out`](../../examples/fan_out).
