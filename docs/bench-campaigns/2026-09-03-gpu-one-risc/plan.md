...
at `bind.rs:3052-3055` — card 6.2 inverts that assertion, `#[cfg]`-paired, as the
...
- `BoundOpKind`: Elementwise{body: ComposedBody, operands} | Reduce{element_body, reduce_op, init, keep, operands, output_axes, out_layout: Layout, out_scatter: Option<Lookup>} | Iota | Constant{value} (`bind.rs:221-264`). `Layout{base: i64, strides}` (`:95-98`). `MAX_INLINE_RANK = 4` (`:82`).
...
- the MILLI rung hardcodes `max_tokens = 5` (`bind.rs:3105`) and sets
...
- the MILLI rung is a 5-token cell (`bind.rs:3103`) with inert env vars — every milli row is
