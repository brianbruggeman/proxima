...
| 2026-09-11 | measured the landed CUDA path on Qwen2.5 0.5B before changing synchronization | CUDA was semantically correct for the Paris smoke at batch/ubatch `1/1` and `32/32`; measured TTFT/TTNT were `7811.258/1098.678 ms` and `1491.307/1093.687 ms`, respectively. The runtime currently synchronizes once per emitted graph node, so this is a host/device serialization baseline, not a kernel-throughput baseline | `bench_local`, 2-token and 8-token runs, local RTX 2070, driver `610.57.04`, NVRTC/runtime paths; no core dump | local RTX 2070 |
| 2026-09-11 | enqueued ordinary CUDA graph kernels and synchronized once per evaluation; measured the explicit relaxed-numerics control | correctness stayed exact for the Paris smoke, but TTNT was `1048.197 ms` with the async path versus `1093.687 ms` baseline; `PROXIMA_RELAXED_NUMERICS=1` was `1042.802 ms`, so neither reaches the 20 tok/s target. The dominant measured boundary is therefore kernel/graph work, not per-node synchronization | two controlled 8-token runs, batch/ubatch `32/32`; 705 resolved nodes in the one-token trace; no core dump | local RTX 2070 |
| 2026-09-11 | enabled CUDA warp-cooperative packed reductions only under `NumericPolicy.reassociation` | relaxed Q4_K execution remained semantically correct for the Paris smoke, but TTNT was `1005.062 ms` (~1 tok/s), so quantized reduction reassociation is not the required 20+ tok/s path. Bit-exact mode remains serial and unchanged | `PROXIMA_RELAXED_NUMERICS=1`; 8 generated tokens; batch/ubatch `32/32`; generated IDs exactly matched the CPU/reference continuation; no core dump | local RTX 2070 |
| 2026-09-11 | instrumented CUDA node execution and result readback separately | one-token trace measured `633–706` emitted nodes and `614–646 ms` accumulated per-node synchronized execution; reading `72–73` output tensors took only `0.396–0.518 ms`. The dominant boundary is kernel execution/launch work, not layer-output readback | `PROXIMA_CUDA_TIMING=1`; one-token Paris smoke; timing mode intentionally synchronizes per node and is diagnostic only; no core dump | local RTX 2070 |
| 2026-09-11 | ran the available Qwen2.5 1.5B decoder through CUDA after async launch changes | batch/ubatch `32/32` produced the correct Paris continuation `[12095, 13]`; measured TTFT `2095.551 ms`, TTNT `975.627 ms`, with CUDA memory `7152384/7980864 KiB` free/total before and after | `bench_local`, 2 generated tokens, local RTX 2070, NVRTC/runtime paths; no coredump | local RTX 2070 |
