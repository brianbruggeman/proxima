...
mod linux {
    ...
    pub fn run() {
        ...
        for index in 0..BLOCK {
            unsafe {
                core::ptr::write_volatile((write_va + index) as *mut u8, (index as u8) ^ 0x5A);
            }
        }
        ...
        let written =
            block_on(Pipe::call(&queue, rw(OPC_WRITE, 0x10, write_pa))).expect("write call");
        assert!(
...
fn main() {
    println!("uio_rw needs a linux host: it drives /sys/bus/pci and /proc/self/pagemap");
}
