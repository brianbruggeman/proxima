...
fn main() -> Result<(), ProximaError> {
    ...
    let addr_text = args.next().unwrap_or_else(|| "127.0.0.1:8094".to_string());
    ...
    // BENCH_PART_SOURCE=1 routes request HEADERS through the lazy PartSource
    // (C35 server half) — inert unless built with the h3-part-source feature.
    let part_source = std::env::var_os("BENCH_PART_SOURCE").is_some();
    let spec = spec.with_spec(json!({ "dev_self_signed": true, "part_source": part_source }));
    let _server = spec.attach(into_handle(OkHandler)).run_with_runtime(
        &registry,
