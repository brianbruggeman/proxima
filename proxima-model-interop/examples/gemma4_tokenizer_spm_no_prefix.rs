...
/// spaces for the SentencePiece marker but do not prepend one, matching
/// gemma4's real `tokenizer.ggml.add_space_prefix = false` metadata.
...
fn main() -> Result<(), InteropError> {
    ...
    for text in test_strings {
        ...
        println!(
            "  spm, add_space_prefix=false (no-regex)    ids = {spm_ids:?} pieces = {:?}",
            spm_ids
                .iter()
