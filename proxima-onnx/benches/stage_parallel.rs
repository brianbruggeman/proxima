...
use tile_pipeline::{
    ...
    run_pipeline_forward, run_pipeline_forward_direct,
};
...
fn conv_stage_worker<const BLOCKED: bool, const ROWS: usize>(
    ...
    loop {
        match spin_recv(&input) {
            StageMsg::Band(band) => {
                let active = stage.get_or_insert_with(|| {
                    ConvReluStage::new(
                        channels_in,
                        channels_out,
                        kernel_height,
...
fn fc_stage_worker(
    input: Arc<Ring<StageMsg>>,
    results: Arc<Ring<[f32; 10]>>,
    weights: MnistWeights<'static>,
) {
    ...
    loop {
        match spin_recv(&input) {
            StageMsg::Band(band) => {
                let active = stage.get_or_insert_with(|| {
                    FcAccumulateStage::new(24, 22, 22, 32, weights.fc1_weight, weights.fc1_bias)
                });
            ...
            StageMsg::EndOfImage => {
                let active = stage
                    .take()
                    .expect("fc stage saw EndOfImage with no bands accumulated");
...
fn run_stage_parallel(images: &[Vec<f32>], weights: MnistWeights<'static>) -> RunResult {
    ...
    let results = Arc::new(Ring::<[f32; 10]>::with_capacity(
        images.len().next_power_of_two().max(2),
    ));
    ...
    let handle_fc = {
        ...
        thread::Builder::new()
            .name("fc1".into())
            .spawn(move || fc_stage_worker(input, output, weights))
            .expect("spawn fc1 worker")
    };
    ...
    let start_ring = Arc::new(Ring::<Instant>::with_capacity(
        images.len().next_power_of_two().max(2),
    ));
...
fn run_data_parallel(
    ...
    for handle in handles {
        let (mut chunk_outputs, mut chunk_latencies) =
            handle.join().expect("data-parallel worker panicked");
...
fn leak_weights(owned: Vec<(String, Vec<f32>)>) -> MnistWeights<'static> {
    ...
    let slices: Vec<(&'static str, &'static [f32])> = leaked
        .iter()
...
fn main() {
    if !checkpoint_present() || !dataset_present() {
        eprintln!(
            "stage_parallel bench: skipping, no host-local mnist.onnx checkout or MNIST idx dataset"
        );
        return;
    }
    ...
    println!(
        "stage_parallel bench: {} images loaded, band={:?}",
        images.len(),
        BAND
    );
    ...
    let reference: Vec<[f32; 10]> = images
        .iter()
        .map(|image| run_pipeline_forward(image, &weights, BAND))
        .collect();
    let direct_match = images
        .iter()
        .zip(&reference)
        .filter(|(image, expected)| {
            run_pipeline_forward_direct(image, &weights, BAND) == **expected
        })
        .count();
    assert_eq!(
        direct_match,
        images.len(),
        "run_pipeline_forward_direct diverged from run_pipeline_forward on this image set"
    );

    let stage_parallel = run_stage_parallel(&images, weights);
    let stage_parallel_match = stage_parallel
        .outputs
        .iter()
        .zip(&reference)
        ...
        .count();
    println!(
        "stage_parallel correctness: {stage_parallel_match}/{} bit-identical to run_pipeline_forward",
        images.len()
    );
    assert_eq!(
        stage_parallel_match,
        images.len(),
        "stage_parallel arm diverged from the single-thread reference"
    );

    let data_parallel = run_data_parallel(&images, weights, 4);
    let data_parallel_match = data_parallel
        .outputs
        .iter()
        .zip(&reference)
        .filter(|(actual, expected)| *actual == *expected)
        .count();
    println!(
        "data_parallel correctness: {data_parallel_match}/{} bit-identical to run_pipeline_forward",
        images.len()
    );
    assert_eq!(
        data_parallel_match,
        images.len(),
        "data_parallel arm diverged from the single-thread reference"
    );
    ...
    let faster_label = if faster_is_data_parallel {
        "data_parallel(4)"
    } else {
        "stage_parallel"
    };
    let full_outputs = if faster_is_data_parallel {
        run_data_parallel(&full_images, weights, 4).outputs
    } else {
        run_stage_parallel(&full_images, weights).outputs
    };
    let correct = full_outputs
        ...
        .count();
    println!(
        "full-1000 accuracy on faster arm ({faster_label}): {correct}/{} = {:.4}",
        full_images.len(),
        correct as f64 / full_images.len() as f64
    );
}
