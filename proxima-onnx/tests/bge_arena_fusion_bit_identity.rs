...
use proxima_tensor::cpu::{
    arena_packed_node_count, build_static_arena_with_constants, epilogue_fuse_reset, evaluate_named, evaluate_named_with_arena,
    layer_norm_cluster_reset, layer_norm_cluster_totals, rewrite_engine_depth_fires, rewrite_engine_reset,
};
...
fn named_inputs<'a>(initializers: &'a [(String, Vec<f32>)], graph_inputs: &'a [String], input_ids: &'a [f32], attention_mask: &'a [f32], token_type_ids: &'a [f32]) -> Vec<(&'a str, &'a [f32])> {
...
fn arena_fusion_matches_evaluate_named_fused_bit_for_bit_on_real_bge() {
    ...
    for (label, tokens) in &items {
        ...
        let named = named_inputs(&lowered.initializers, &lowered.graph_inputs, &input_ids, &attention_mask, &token_type_ids);

        // Arm A: `evaluate_named` -- today's sealed fused-only baseline.
        let evaluated_a = evaluate_named(&lowered.program, &[], &named, &[output]).expect("evaluate_named on the real BGE graph");
        let (data_a, shape_a) = evaluated_a.get(output).expect("arm A last_hidden_state present");
        assert_eq!(shape_a, &[1u64, sequence_length as u64, 384u64], "{label}: unexpected arm A shape");
        ...
        let constant_inputs: Vec<(&str, &[f32])> = lowered.initializers.iter().map(|(name, data)| (name.as_str(), data.as_slice())).collect();
        let mut arena = build_static_arena_with_constants(&lowered.program, &[], &[output], &constant_inputs).expect("build packed+fused arena");
        arena_builds += 1;
        ...
        let evaluated_c = evaluate_named_with_arena(&mut arena, &named).expect("evaluate_named_with_arena on the real BGE graph");
        ...
        let (data_c, shape_c) = evaluated_c.get(output).expect("arm C last_hidden_state present");
        assert_eq!(shape_c, &[1u64, sequence_length as u64, 384u64], "{label}: unexpected arm C shape");
        ...
        assert!(
            data_a.iter().zip(data_c.iter()).all(|(&left, &right)| left.to_bits() == right.to_bits()),
    ...
    assert_eq!(arena_builds, items.len(), "expected exactly one arena build per distinct pinned sentence length, not per call");
    assert!(any_packed, "engagement N==0 is RED: no width-tile node was packed on the real BGE graph");
    assert!(depth1_fires_total > 0, "engagement N==0 is RED: law 1/2 admission never fired at arena-build time");
    assert!(depth2_fires_total > 0, "engagement N==0 is RED: law 2's layer-norm cluster upgrade never fired at arena-build time");
    assert!(any_ln_cluster_hits > 0, "engagement N==0 is RED: layer-norm cluster fusion never fired at runtime in the arena path");
    ...
    println!("arena-path cosine(A,B)={similar:.6} cosine(A,C)={dissimilar_a:.6} cosine(B,C)={dissimilar_b:.6}");
    assert!((similar - 0.936311).abs() < 1e-5, "cosine(A,B) drifted from the sealed oracle: {similar:.6}");
    assert!((dissimilar_a - 0.378777).abs() < 1e-5, "cosine(A,C) drifted from the sealed oracle: {dissimilar_a:.6}");
    assert!((dissimilar_b - 0.334176).abs() < 1e-5, "cosine(B,C) drifted from the sealed oracle: {dissimilar_b:.6}");
}
