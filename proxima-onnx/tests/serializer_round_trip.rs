...
use proxima_onnx::messages::{
    ...
    OperatorSetIdProto, TensorProto, TensorShapeProto, TypeProto, TypeProtoTensor, TypeValue,
    ValueInfoProto,
};
...
fn serializer_builds_a_two_layer_mlp_that_reparses_lowers_and_evaluates_correctly() {
    ...
    let softmax_axis = AttributeProto {
        name: "axis",
        i: 1,
        type_raw: 2,
    ...
    assert!(
        !bytes.is_empty(),
        "the serializer produces nonempty wire bytes"
    );

    let reparsed_model = proxima_onnx::pipe::parse_complete(&bytes)
        .expect("re-parse the serializer's own wire bytes");
    let reparsed_graph = reparsed_model
        .graph
        .as_ref()
        .expect("re-parsed model carries a graph");
    assert_eq!(
        reparsed_graph.node.len(),
        4,
        "all four nodes round-trip through the serializer"
    );

    let lowered =
        proxima_onnx::lower::lower_graph(reparsed_graph).expect("lower the serializer-built graph");
    ...
    let evaluated =
        proxima_tensor::cpu::evaluate_named(&lowered.program, &[], &named, &[output_node])
            .expect("evaluate the serializer-built program");
