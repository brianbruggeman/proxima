...
fn real_mnist_onnx_parses_lowers_and_evaluates_end_to_end() {
    ...
    let lowered = proxima_onnx::lower::lower_graph(graph).expect(
        "lower the real mnist.onnx graph to Op (BatchNormalization + LogSoftmax now supported)",
    );
    std::println!(
        "real_mnist lowered program length: {}",
        lowered.program.len()
    );
    assert!(
        !lowered.program.is_empty(),
        "a real model lowers to a nonempty Op program"
    );
    ...
    assert!(
        ...
        "every log-probability is finite, got {data:?}"
    );
    let probability_mass: f32 = data
        .iter()
        .map(|log_probability| log_probability.exp())
        .sum();
    assert!(
        (probability_mass - 1.0).abs() < 1e-3,
        "exp(log-softmax) sums to 1 (a valid probability distribution), got {probability_mass}"
    );
}
