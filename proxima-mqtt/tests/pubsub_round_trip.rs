...
use proxima_mqtt::{
    MqttBroker, MqttPipeReply, MqttPipeRequest, MqttServerConfig, serve_connection,
};
use proxima_net::tokio::tokio_stream_listener::TokioTcpConnection;
use proxima_primitives::pipe::SendPipe;
use proxima_primitives::pipe::request::Response;
use proxima_protocols::mqtt::encode::{
    encode_connect, encode_publish, encode_subscribe, encode_unsubscribe,
};
use proxima_protocols::mqtt::{MqttReply, MqttRequest, Packet, ParseError, parse_packet};
        ...
        match parse_packet(buffered) {
            Ok((packet, consumed)) => {
                let inbound = match packet {
                    ...
                    Packet::SubAck { return_codes, .. } => Inbound::SubAck {
                        granted: return_codes.to_vec(),
                    },
                    Packet::Ack { .. } => Inbound::UnsubAck,
                    Packet::Publish { topic, payload, .. } => Inbound::Publish {
                        topic: topic.to_vec(),
...
async fn publish_delivers_to_a_subscribed_connection_across_two_sockets() {
    ...
    assert_eq!(
        connect(&mut subscriber, &mut subscriber_buf, "subscriber").await,
        0
    );
    ...
    assert_eq!(
        connect(&mut publisher, &mut publisher_buf, "publisher").await,
        0
    );
...
async fn unsubscribed_topic_no_longer_receives_publishes() {
    ...
    assert_eq!(
        &chunk[..read],
        &[0xD0, 0x00],
        "only the PINGRESP arrived, no leaked PUBLISH"
    );
}
