//! `MqttConnSource<R>` — the caller-supplied `UnpinPipe<In = (), Out =
...
//! never `Sync` — and `MqttConnectionPipe: SendPipe`'s upgrade closure
...
//! retrofit. `&FanIn<MqttConnSource<..>, ..>` crossing an `.await` demands
//! `MqttConnSource: Sync`, which `RefCell` can never provide. `Mutex` is
...
// torn state.
impl<R> DropSafe for MqttConnSource<R> {}

/// The future behind [`MqttConnSource::call`] — one hand-written `poll`
...
impl<R: AsyncRead + Unpin> Future for MqttConnCall<'_, R> {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        match self.get_mut() {
            ...
            MqttConnCall::Shutdown { receiver } => {
...
impl<R: AsyncRead + Unpin> UnpinPipe for MqttConnSource<R> {
    type In = ();
    type Out = WireEvent;
    type Err = Exhausted;
...
mod tests {
    ...
    fn read_source_yields_bytes_then_stop_on_eof() {
        let source = MqttConnSource::read(
            OnceReader {
                chunk: b"\x10\x00".to_vec(),
                served: false,
            },
            64,
        );
        ...
        match first {
            WireEvent::Read(bytes) => assert_eq!(&bytes[..], b"\x10\x00"),
    ...
    fn read_source_surfaces_a_real_io_error_as_failed_not_stop() {
        let source = MqttConnSource::read(FailingReader, 64);
    ...
    fn push_source_yields_a_pushed_item() {
        ...
        let outcome = block_on(UnpinPipe::call(&source, ())).expect("poll");
        match outcome {
            WireEvent::Push(bytes) => assert_eq!(&bytes[..], b"\x30\x00"),
    ...
    fn shutdown_source_fires_stop_once_signaled() {
        ...
        let source = MqttConnSource::<OnceReader>::shutdown(rx);
