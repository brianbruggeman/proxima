//! `MqttServerConfig` — the facade's config-mirror surface (workspace
//! principle 4): one type is the bon builder result, the serde shape, and
//! the conflaguration env surface (`MQTT_*`). Mirrors
...
pub struct MqttServerConfig {
    ...
    /// write buffer flush threshold; replies (and pushed pub/sub
    /// `PUBLISH` frames) accumulate in the connection's out buffer up to
    /// this many bytes before a socket write
    #[setting(default = 65536)]
    #[serde(default = "default_high_water")]
    #[builder(default = default_high_water())]
    pub write_high_water_bytes: usize,

    /// hard cap on one still-incomplete inbound packet — the DoS guard
...
impl Validate for MqttServerConfig {
...
mod tests {
    ...
    fn builder_overrides_defaults() {
        let config = MqttServerConfig::builder()
