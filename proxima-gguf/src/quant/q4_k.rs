...
pub const BLOCK_BYTES: usize = {
    ...
    assert!(
        ...
        "GgmlType::Q4_K block_elements drifted from QK_K"
    );
    ...
    for _ in 0..QK_K / 64 {
        ...
        for offset in 0..SUB_BLOCK_ELEMENTS {
            output[out_offset + offset] =
                scale_lo * f32::from(qs[qs_offset + offset] & 0x0F) - min_lo;
...
fn quantize_block(x: &[f32], output: &mut [u8]) {
    ...
    for base in (0..QK_K).step_by(64) {
        for offset in 0..SUB_BLOCK_ELEMENTS {
            qs[qs_offset + offset] =
                levels[base + offset] | (levels[base + SUB_BLOCK_ELEMENTS + offset] << 4);
        }
        qs_offset += SUB_BLOCK_ELEMENTS;
    }
}
...
mod tests {
    ...
    use super::{
        BLOCK_BYTES, CODEC, K_SCALE_SIZE, QK_K, QuantError, SUB_BLOCK_ELEMENTS, SUB_BLOCKS,
        dequantize, quantize,
    };
    ...
    fn quantize_dequantize_constant_nonzero_vector_is_near_exact() {
        ...
        let max_error = output
            ...
            .fold(0.0f32, f32::max);
        debug!(max_error, "quant.q4_k constant-nonzero-vector round trip");
        assert!(
            max_error < 0.01,
    ...
    fn quantize_dequantize_smooth_signal_round_trip_error() {
        ...
        debug!(max_error, rms_error, "quant.q4_k smooth-signal round trip");
        assert!(
            max_error < 0.6,
    ...
    /// signal: only the sub-block holding the outlier gets a nonzero
    /// scale, the rest stay on the `max == min` fast path.
    #[test]
    fn quantize_dequantize_single_outlier_block() {
        ...
        let max_error = output
            ...
            .map(|(got, want)| (got - want).abs())
            .fold(0.0f32, f32::max);
        debug!(max_error, "quant.q4_k single-outlier round trip");
    ...
    fn quantize_dequantize_alternating_sign_block() {
        ...
        let max_error = output
            ...
            .map(|(got, want)| (got - want).abs())
            .fold(0.0f32, f32::max);
        debug!(max_error, "quant.q4_k alternating-sign round trip");
        assert!(max_error < 0.6, "max_error={max_error} exceeds loose sanity bound");
    }
    ...
    /// Real weight data (principle 9): the checkpoint's own
    /// `token_embd.weight` dequantized to `f32`, re-encoded through
    /// `Q4_K`. Bound is the same loose sanity envelope the smooth-signal
    /// test above already established for this codec; measured RMS is
    /// logged, not hidden.
    #[cfg(feature = "std")]
    #[test]
    fn quantize_dequantize_real_qwen3_weights_round_trip_error() {
        ...
        debug!(max_error, rms_error, "quant.q4_k real-qwen3-weights round trip");
        assert!(max_error < 0.6, "max_error={max_error} exceeds loose sanity bound");
