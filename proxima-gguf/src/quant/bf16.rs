...
mod tests {
    ...
    /// a pure bit-truncation, so this is really a compile-time guarantee,
    /// but asserted per Q1's shared contract across every codec.
    #[test]
    fn quantize_is_deterministic_across_repeated_calls() {
        let input = vec![1.0f32, -2.5, 0.0, 128.25, -0.125];
    ...
    /// `encode(dequant(encode(x))) == encode(x)`: bf16 truncation is
    /// already idempotent at its own grid by construction, asserted here
    /// rather than assumed.
    #[test]
    fn quantize_is_idempotent_at_the_codec_grid() {
        let input = vec![1.0f32 + 2.0f32.powi(-7) + 2.0f32.powi(-9), -3.0, 42.0];
