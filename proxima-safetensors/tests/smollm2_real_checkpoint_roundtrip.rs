...
use proxima_safetensors::{
    Manifest, SafetensorsModel, SafetensorsParser, TensorPayload, write_complete,
};
...
fn write_complete_round_trips_a_real_tensors_real_bytes_byte_identical() {
    ...
    let Some((manifest, data_start, mut file)) = real_manifest(path) else {
        eprintln!(
            "real safetensors file not found at {REAL_SMOLLM2_SAFETENSORS_PATH}; test skipped"
        );
        return;
    };
    ...
    assert_eq!(
        ...
        Some("pt"),
        "real __metadata__ must round-trip through our own reader"
    );
    ...
    file.seek(SeekFrom::Start(data_start + entry.data_offsets.0))
        .expect("seek to real tensor data");
    ...
    assert!(
        real_bytes.iter().any(|&byte| byte != 0),
        "real tensor bytes must not be degenerate all-zero"
    );
    ...
    let reparsed = proxima_safetensors::parse_complete(&written)
        .expect("writer's own output re-parses through our reader");
    let reparsed_entry = reparsed
        .tensor(TARGET_TENSOR)
        .expect("re-parsed manifest carries the tensor back");
    ...
    assert_eq!(
        reparsed_entry.byte_len(),
        entry.byte_len(),
        "byte_len must round-trip"
    );
    assert_eq!(
        reparsed.metadata.get("format").map(String::as_str),
        Some("pt"),
        "metadata must round-trip through the writer"
    );

    let written_header_len =
        u64::from_le_bytes(written[..8].try_into().expect("8-byte length prefix")) as usize;
    let written_data_start = 8 + written_header_len;
    let (start, end) = reparsed_entry.data_offsets;
    let round_tripped_bytes =
        &written[written_data_start + start as usize..written_data_start + end as usize];
