...
//! `proxima-onnx::OnnxParser` satisfy. [`Self::poll`] computes the new
//! variant (`Header` -> `TensorData`) as an owned value first, from data
//! already read out of the old variant, then assigns `*self` to it in one
//! step — no `unsafe`, no placeholder, no extra allocation, and the
//! enum-folded cursor invariant `DelimiterFraming` motivates is unchanged;
//! only the public boundary moved from self-consuming to `&mut self`.
//! [`Self::push`] stays as a convenience built on `feed`/`poll` for callers
//! that prefer threading an owned `Self` through a fold — it is sugar now,
//! not the primitive.
//!
...
impl SafetensorsParser {
    ...
    /// through a fold instead of holding a `&mut` binding.
    ...
    /// Signal end of input. Read-only: validates every tensor's
    /// `data_offsets` against the bytes actually counted, or reports the
    /// header as truncated if it never completed. Use [`Self::into_manifest`]
    /// to consume the parser and get the finished [`Manifest`] back once
    /// this returns `Ok(())`.
