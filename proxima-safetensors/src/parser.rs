...
//! self-consuming `Self -> Self` transition, not a [`Pipe`](proxima_primitives::pipe::Pipe) —
//! see `crate::header_codec` for why that stateless one-shot step is the
//! `Pipe`, and this stateful accumulation loop is not.
//!
        ...
        match self {
            Self::Header { mut buf } => {
                ...
                match HeaderCodec.parse_frame(&buf) {
                    ...
                    Err(SafetensorsError::TruncatedInput { .. }) => Ok(Self::Header { buf }),
                    Err(error) => Err(error),
                }
            }
