 ...
 * a data-abort exit instead of touching real memory. Chosen well above
 * `GUEST_MEMORY_SIZE` (64 MiB, `dispatch.rs::run_dispatch_loop`) so it can
 * never overlap a real ELF segment or the stack reservation. Window size
 * covers every register through `ConfigGeneration` (0x0fc) plus headroom
 * for device-specific config space (0x100+), never itself dereferenced by
 ...
 * above `GUEST_MEMORY_SIZE` so it can never alias real guest RAM. Sized
 * identically for the same headroom reason (register block through
 * `ConfigGeneration` plus the net device's own config-space fields,
 ...
 * range the guest was told to expect. Both sit below `GUEST_MEMORY_SIZE`'s
 * upper bound in guest-physical-address terms only in the sense that they are
 * far ABOVE the guest RAM region actually mapped in this VM
 * (`dispatch::run_dispatch_loop` maps guest RAM at `[0, guest_memory_size)`,
 * 64 MiB, `0x0400_0000`): `0x0800_0000` (GICD) and `0x080a_0000` (GICR) both
 * exceed that ceiling, so neither window is ever backed by `hv_vm_map`'d
 * guest memory and every guest access genuinely traps as a data abort, the
 * same non-aliasing guarantee the virtio windows get from sitting at
 * `0x1000000000+` instead. */
 ...
 * `0x0900_0000` exceeds `GUEST_MEMORY_SIZE` (`dispatch::run_dispatch_loop`'s
 * 64 MiB / `0x0400_0000` ceiling), so this window is never backed by
 * `hv_vm_map`'d guest memory either, and every guest access genuinely traps
 * as a data abort. */
 ...
 * trap loop (`proxima_vm_run_dispatch_loop` below) range-tests `x0` against
 * `PROXIMA_VM_PSCI_FAST_CALL_32_BASE`/`_64_BASE` (each a 0x20-wide SMCCC
 * fast-call range) BEFORE its `PROXIMA_VM_HALT_VERB`/`PROXIMA_VM_EMIT_VERB`
 * checks, since a compliant PSCI function ID is six orders of magnitude
 * above either sentinel and never collides with them (`src/psci.rs`'s own
 * module doc walks the disjointness argument). `*return_value_out` carries
 * the signed 32-bit value (sign-extended to 64 bits) to write into the
 * guest's `x0` before resuming, valid only when `*action_out == 0`.
 * `*action_out`: `0` resume the guest with `*return_value_out` in `x0`, `1`
 * `SYSTEM_OFF` — end the dispatch loop exactly like `PROXIMA_VM_HALT_VERB`
 * (never a second exit channel), `2` `SYSTEM_RESET` — this handler has no
 * reset capability, so it ends the loop identically to `SYSTEM_OFF` rather
 * than returning success into a guest that would then run past it. Returns
 * 0 always; failure is not a concept `src/psci.rs`'s pure match produces. */
 ...
 * guest address space, whose guest-physical base is `guest_memory_base`
 * (0 for the existing ELF-guest callers, `dtb.rs`'s RAM base for a real
 * kernel boot) — each at its own `guest_address` relative to that base, its
...
/* `console_transport` is a live `ConsoleTransport` (`src/virtio_console.rs`),
 * `net_transport` a live `NetTransport` (`src/virtio_net.rs`), and
 * `blk_transport` a live `BlkTransport` (`src/virtio_blk.rs`); this loop
 * routes every data-abort exit whose fault address falls inside the console
 * window (`[PROXIMA_VM_MMIO_WINDOW_BASE, +PROXIMA_VM_MMIO_WINDOW_SIZE)`), the
 * net window (`[PROXIMA_VM_NET_MMIO_WINDOW_BASE,
 * +PROXIMA_VM_NET_MMIO_WINDOW_SIZE)`), or the blk window
 * (`[PROXIMA_VM_BLK_MMIO_WINDOW_BASE, +PROXIMA_VM_BLK_MMIO_WINDOW_SIZE)`) to
 * the matching transport's dispatch function; a `QueueNotify` effect
 ...
 * `net_emitted_out` (net), or `blk_emitted_out` (blk) — four channels
 * deliberately kept separate from each other and from `emitted_out` (the
 * `PROXIMA_VM_EMIT_VERB` hypercall path) so a guest exercising every path in
 * one run (as `guests/lambda/src/main.rs` does) never has its observable
 * byte streams silently interleaved into one buffer a test would then have
 * to disentangle. A fifth channel, `pl011_emitted_out`, carries the bytes a
 * guest writes to `UARTDR` inside the pl011 window
 * (`[PROXIMA_VM_PL011_MMIO_WINDOW_BASE, +PROXIMA_VM_PL011_MMIO_WINDOW_SIZE)`)
 * — this VM's console byte channel M5's exit criterion names, kept
 * deliberately separate from every channel above for the same
 * never-silently-interleaved reason. */
/* M3 fault-count instrument (`tools/proxima-vm/ROADMAP.md`, "M3 — the
 * fault-count instrument"): three additional out-params, all optional
 * (NULL-safe on the caller's side is NOT assumed here — callers always pass
 * a live pointer, matching every other `*_out` parameter above).
 *
 * `create_to_first_exit_nanos_out`: wall time from this function's own
 * entry (before any hypervisor call) to the first successful vCPU-run exit
 * — the earliest host-observable point the guest has executed at least one
 * instruction. Read via a monotonic clock at the C level so the timing
 * brackets the exact hypervisor calls, never estimated from the Rust side
 * of the FFI boundary.
 *
 * `touch_all_pages_nanos_out`: wall time to write one byte into every
 * `page_size` stride of the freshly `mmap`'d guest memory, BEFORE any
 * `hv_vm_map`/`KVM_SET_USER_MEMORY_REGION` call. This is a host-mmap
 * first-touch cost, not a stage-2 hypervisor fault count — HVF has no
 * stage-2 fault-index equivalent to KVM's `pagemap` soft-dirty bits (see
 * the module doc on `backend_macos.c`'s implementation), so this wall-time
 * number is the HVF lane's whole M3 deliverable for this axis.
 *
 * `mmio_trap_count_out`: count of data-abort (`backend_macos.c`) /
 * `KVM_EXIT_MMIO` (`backend_linux.c`, unwired — always 0 here) exits
 * serviced during the run. Every data abort in this design is, by
 * construction, an access to a deliberately-unmapped virtio-mmio window
 * (guest RAM is always eagerly `hv_vm_map`'d up front, never demand-paged),
 * so this is an MMIO-trap count, not a stage-2 RAM-fault count — it must
 * never be read as the M3 fault-count column.
 *
 * M5b per-window breakdown of `mmio_trap_count_out`, attributed by the same
 * window resolution `handle_mmio_data_abort` already performs:
 * `gicd_trap_count_out`, `gicr_trap_count_out`, `pl011_trap_count_out` (the
 * uart window this M5b earlycon investigation reads directly), and
 * `virtio_trap_count_out` (console+net+blk combined — this boot's guest
 * never drives any of the three, so a further split would only show
 * zeros). `vtimer_activation_count_out` counts
 * `HV_EXIT_REASON_VTIMER_ACTIVATED` exits serviced (0 or 1 in practice: HVF
 * auto-masks the vtimer on the first one and this loop never unmasks it, so
 * a second activation would only occur if a future slice adds interrupt
 * injection and services the guest's EOI). All five are unconditional
 * out-params, populated in `cleanup` regardless of `result`, same as
 * `mmio_trap_count_out` itself — the whole point is diagnosing a run that
 * did NOT reach a clean halt. */
/* M4 — guest memory as a named object. `proxima_vm_create_named_region`
 * creates the backing object (`memfd_create`+`ftruncate` on KVM,
 * `vm_allocate`+`mach_make_memory_entry_64` on HVF), maps its first view at
 * `region_out->primary_address`, and zero-fills it. Returns 0 on success,
 * -1 on failure (message in `error_buffer`).
 *
 * `proxima_vm_map_named_region` opens a SECOND, independent host-address-
 * space view of the same backing object named by `region->handle`.
 * `want_private_view == 0` maps it `MAP_SHARED`-equivalent: writes through
 * either view become visible through the other. `want_private_view == 1`
 * maps it `MAP_PRIVATE`-equivalent (KVM lane only -- the HVF lane has no
 * mach-memory-entry copy-on-write primitive this simple and returns -1
 * naming that gap in `error_buffer`): a write through the private view
 * never becomes visible through `region->primary_address` or any other
 * view, and a read through the private view before its own first write
 * still observes whatever `primary_address` held at map time. Returns 0 on
 * success (`*host_address_out` set), -1 on failure.
 *
 * `proxima_vm_unmap_named_region_view` unmaps one view created by either
 * function above WITHOUT closing `region->handle` -- the backing object
 * survives so other views stay valid. `proxima_vm_destroy_named_region`
 * unmaps the primary view and closes the handle, retiring the object. */
...
int proxima_vm_run_dispatch_loop(
     ...
     * CPSR (`backend_macos.c::proxima_vm_run_dispatch_loop`'s own doc on
     * this parameter, and `boot::boot_edk2_firmware`'s doc on why edk2
     * needs EL2h instead). */
    uint64_t boot_cpsr,
    const void *dispatcher,
    void *console_transport,
     ...
     * requested and this host fell back to EL1h -- `create_vm`'s own doc
     * in `backend_macos.c` on why HV_UNSUPPORTED triggers a same-call
