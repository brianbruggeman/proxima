...
| AC8 | R6 (confinement) | `git -C $WT diff 5ec9c8698 --name-only\|grep -vE '^(omega/\|proxima-tensor/)'\|grep -vcE '/tests?/'` | `0` NON-test files outside omega/proxima-tensor. A forced exhaustive-match arm in a consumer-crate test (e.g. `proxima-model-interop/tests/`) is EXPECTED and permitted — the type-safe `RoundBatchedReduce` variant makes the compiler touch every `BoundOpKind` consumer, which is the safety, not sprawl |
