...
fn emit_sizing_consts() {
    ...
    let parallel_threshold = require_nonzero(
        "parallel.threshold",
        resolve_int(&root, "parallel", "threshold"),
    );
    ...
    let cohort_spin_polls = require_nonzero(
        "cohort.spin_polls",
        resolve_int(&root, "cohort", "spin_polls"),
    );
    ...
    if env::var_os("CARGO_FEATURE_COHORT_STAGED_GRAPH").is_some() {
        ...
        out.push_str(&format!(
            "pub const STAGED_BATCH_MIN_LEN: usize = {staged_batch_min_len};\n"
        ));
    }
