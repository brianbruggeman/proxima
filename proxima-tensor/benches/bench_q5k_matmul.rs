...
fn bench_shape(c: &mut Criterion, label: &str, tensor_name: &str, seed: u64) {
    ...
    let ours_dequant =
        matmul_q5k_f32(&weight_bytes, out_dim, &activation).expect("well-formed real q5_k matmul");
    ...
    #[cfg(feature = "q5k-int8-dot")]
    {
        let ours_packed = matmul_q5k_q8k_f32(&weight_bytes, out_dim, &activation)
        ...
        let ours_portable = matmul_q5k_q8k_portable_f32(&weight_bytes, out_dim, &activation)
        ...
        c.bench_function(
            &format!("{label}_proxima_matmul_q5k_q8k_dispatched_t1"),
            |b| {
                b.iter(|| {
                    black_box(matmul_q5k_q8k_f32(&weight_bytes, out_dim, &activation).unwrap())
                })
            },
        );
        c.bench_function(
            &format!("{label}_proxima_matmul_q5k_q8k_portable_t1"),
            |b| {
                b.iter(|| {
                    black_box(
                        matmul_q5k_q8k_portable_f32(&weight_bytes, out_dim, &activation).unwrap(),
                    )
                })
            },
        );
