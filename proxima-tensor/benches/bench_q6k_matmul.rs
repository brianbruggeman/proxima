//! `proxima_tensor::cpu::{matmul_q6k_f32, matmul_q6k_q8k_f32,
//! matmul_q6k_q8k_portable_f32}` vs ggml's `ggml_mul_mat` on a
//! `GGML_TYPE_Q6_K` source, batch-1 decode, at `output.weight`
//! (4096x32002) -- the one tensor the real openchat-3.5-1210 `Q4_K_S` file
//! bumps to `Q6_K` per llama.cpp's own mixed-precision quantization
//! recipe.
//!
//! Mirrors `bench_q5k_matmul.rs`'s structure and correctness-before-timing
//! discipline exactly, one format down.
...
use proxima_tensor::cpu::matmul_q6k_f32;
#[cfg(feature = "q6k-int8-dot")]
use proxima_tensor::cpu::{matmul_q6k_q8k_f32, matmul_q6k_q8k_portable_f32};
...
fn parse_header(path: &Path) -> (ParsedGguf, u64) {
    ...
    loop {
        ...
        // a truncated prefix that fails to parse just grows and retries below
...
fn bench_shape(c: &mut Criterion, label: &str, tensor_name: &str, seed: u64) {
    ...
    if tensor.ggml_type != proxima_gguf::types::GgmlType::Q6_K {
        println!(
            "{label} BLOCKED: real file stores this tensor as {:?}, not Q6_K \
    ...
    let ggml_out = unsafe {
        ...
        let weight = ggml_new_tensor_2d(ctx, GGML_TYPE_Q6_K, in_dim as i64, out_dim as i64);
    ...
    let ours_dequant = matmul_q6k_f32(&weight_bytes, out_dim, &activation).expect("well-formed real q6_k matmul");
    ...
    #[cfg(feature = "q6k-int8-dot")]
    {
        let ours_packed = matmul_q6k_q8k_f32(&weight_bytes, out_dim, &activation).expect("well-formed packed int8 matmul");
        ...
        let ours_portable = matmul_q6k_q8k_portable_f32(&weight_bytes, out_dim, &activation).expect("well-formed portable matmul");
        assert_eq!(ours_packed, ours_portable, "{label}: dispatched and portable packed-int8 arms diverged");

        c.bench_function(&format!("{label}_proxima_matmul_q6k_q8k_dispatched_t1"), |b| {
            b.iter(|| black_box(matmul_q6k_q8k_f32(&weight_bytes, out_dim, &activation).unwrap()))
        });
        c.bench_function(&format!("{label}_proxima_matmul_q6k_q8k_portable_t1"), |b| {
            b.iter(|| black_box(matmul_q6k_q8k_portable_f32(&weight_bytes, out_dim, &activation).unwrap()))
        });
    }
    ...
    unsafe {
        ...
        c.bench_function(&format!("{label}_ggml_q6k_mulmat_t1"), |b| {
    ...
    c.bench_function(&format!("{label}_proxima_matmul_q6k_f32_dequant_t1"), |b| {
        b.iter(|| black_box(matmul_q6k_f32(&weight_bytes, out_dim, &activation).unwrap()))
    });
...
fn main() {
    ...
    bench_shape(&mut criterion, "output_weight_4096x32002", "output.weight", 307);
