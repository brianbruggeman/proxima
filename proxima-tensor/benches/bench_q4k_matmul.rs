...
fn main() {
    ...
    // every proxima arm below is labeled `_t1`, but `matmul_worker_count`
    // (cpu.rs) defaults to `hw.perflevel0.logicalcpu` (8 on this host)
    // whenever `PROXIMA_MATMUL_WORKERS` is unset -- without this pin, the
    // `_dispatched_t1`/`_portable_t1` arms silently ran on every performance
    // core, not one thread, and the number drifted with the host's own core
    // count (discipline.md ROW 120): attn_q_4096x4096 measured 119.46us
    // unpinned vs 410.36us pinned on this machine, a ~3.4x gap the label
    // gave no warning of.
