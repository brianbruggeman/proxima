...
unsafe fn dot_q6k_q8k_block_neon_dotprod(weight_block: &[u8], q8k_block: &[u8]) -> f32 {
    ...
    unsafe {
        ...
        // (190 vs 143), which is the signature of dependency depth rather
        // than volume. Integer addition is associative, so splitting the
        // chain is bit-identical, not merely close. The same defect cost 3.2x
        // in `dot_q4k_f32` in an earlier round; the fix there measured 5.68x
        // because width and depth were not independent factors.
        let mut sumi0: i32 = 0;
        let mut sumi1: i32 = 0;
        let mut sumi2: i32 = 0;
        let mut sumi3: i32 = 0;
