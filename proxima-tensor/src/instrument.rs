...
// ---------------------------------------------------------------------
// proxima-debugger DIAGNOSTIC (temporary): per-member round attribution
// for `cpu.rs`'s `matmul_rows_threaded` cohort dispatch.
//
// The existing `WORKER_BUSY_TICKS`/`WORKER_CPU_NANOS` maps answer "how much
// did each puller do", but they are keyed by `ThreadId` behind a `Mutex`
// and carry no round structure, so they cannot answer the three questions
// that decide whether the cohort barrier itself is the serial fraction:
// how late each member reaches its first chunk of a round, how long it
// idles after its last one, and how many members never showed up at all.
// Everything below is slot-indexed and lock-free for exactly that reason —
// a `Mutex` on this path would serialize the thing under measurement.
// ---------------------------------------------------------------------

/// Enough for the 8-worker cohort plus headroom; a slot index past this is
/// clamped into the last slot rather than growing the arrays.
pub const MAX_SLOTS: usize = 16;

static NEXT_SLOT: AtomicU64 = AtomicU64::new(0);
static ROUND_SEQ: AtomicU64 = AtomicU64::new(0);
static ROUND_OPEN_TICK: AtomicU64 = AtomicU64::new(0);

static SLOT_CHUNKS: [AtomicU64; MAX_SLOTS] = [const { AtomicU64::new(0) }; MAX_SLOTS];
static SLOT_BUSY_TICKS: [AtomicU64; MAX_SLOTS] = [const { AtomicU64::new(0) }; MAX_SLOTS];
static SLOT_CPU_NANOS: [AtomicU64; MAX_SLOTS] = [const { AtomicU64::new(0) }; MAX_SLOTS];
static SLOT_FIRST_LATENCY_TICKS: [AtomicU64; MAX_SLOTS] = [const { AtomicU64::new(0) }; MAX_SLOTS];
static SLOT_ROUNDS: [AtomicU64; MAX_SLOTS] = [const { AtomicU64::new(0) }; MAX_SLOTS];
static SLOT_TAIL_TICKS: [AtomicU64; MAX_SLOTS] = [const { AtomicU64::new(0) }; MAX_SLOTS];
static SLOT_LAST_ROUND: [AtomicU64; MAX_SLOTS] = [const { AtomicU64::new(0) }; MAX_SLOTS];
static SLOT_LAST_END_TICK: [AtomicU64; MAX_SLOTS] = [const { AtomicU64::new(0) }; MAX_SLOTS];

static ROUND_COUNT: AtomicU64 = AtomicU64::new(0);
static ROUND_WALL_TICKS: AtomicU64 = AtomicU64::new(0);
static ROUND_ABSENT_TICKS: AtomicU64 = AtomicU64::new(0);
static ROUND_MEMBER_SLOTS: AtomicU64 = AtomicU64::new(0);

std::thread_local! {
    static MY_SLOT: core::cell::Cell<usize> = const { core::cell::Cell::new(usize::MAX) };
}

/// Stable per-thread index into the arrays above, assigned on first touch.
/// The leader takes slot 0 by construction: [`round_open`] runs on the
/// leader thread and is the first call any thread makes.
#[must_use]
pub fn worker_slot() -> usize {
    MY_SLOT.with(|cell| {
        ...
        let assigned = (NEXT_SLOT.fetch_add(1, Ordering::Relaxed) as usize).min(MAX_SLOTS - 1);
        cell.set(assigned);
        assigned
    })
}

/// Publishes a new round and returns its open tick. Called by the leader
/// immediately before `CohortSession::run`, so the returned tick predates
/// the cohort's own `round` bump by a handful of relaxed stores.
#[must_use]
pub fn round_open() -> u64 {
    let _ = worker_slot();
    let now = raw_tick();
    ROUND_OPEN_TICK.store(now, Ordering::Relaxed);
    ROUND_SEQ.fetch_add(1, Ordering::Release);
    now
}

/// Called at the top of every `run_chunk`. Returns this thread's slot and
/// the chunk's start tick; charges round-open-to-first-claim latency the
/// first time a slot is seen in a round.
#[must_use]
pub fn chunk_entry() -> (usize, u64) {
    let slot = worker_slot();
    let now = raw_tick();
    let sequence = ROUND_SEQ.load(Ordering::Acquire);
    if SLOT_LAST_ROUND[slot].swap(sequence, Ordering::Relaxed) != sequence {
        let opened = ROUND_OPEN_TICK.load(Ordering::Relaxed);
        SLOT_FIRST_LATENCY_TICKS[slot].fetch_add(now.wrapping_sub(opened), Ordering::Relaxed);
        SLOT_ROUNDS[slot].fetch_add(1, Ordering::Relaxed);
    }
    (slot, now)
}

/// Called at the bottom of every `run_chunk`, with the thread-CPU delta
/// measured across the same window as `started`.
pub fn chunk_exit(slot: usize, started: u64, cpu_nanos: u64) {
    let now = raw_tick();
    SLOT_BUSY_TICKS[slot].fetch_add(now.wrapping_sub(started), Ordering::Relaxed);
    SLOT_CPU_NANOS[slot].fetch_add(cpu_nanos, Ordering::Relaxed);
    SLOT_CHUNKS[slot].fetch_add(1, Ordering::Relaxed);
    SLOT_LAST_END_TICK[slot].store(now, Ordering::Relaxed);
}

/// Called by the leader immediately after `CohortSession::run` returns —
/// every member has reported done, so every slot's round-local state is
/// final and readable without synchronization beyond the barrier itself.
pub fn round_close(opened_at: u64, members: usize) {
    let now = raw_tick();
    let wall = now.wrapping_sub(opened_at);
    let sequence = ROUND_SEQ.load(Ordering::Relaxed);
    let mut present = 0_u64;
    for slot in 0..MAX_SLOTS {
        if SLOT_LAST_ROUND[slot].load(Ordering::Relaxed) != sequence {
            continue;
        }
        present += 1;
        let last_end = SLOT_LAST_END_TICK[slot].load(Ordering::Relaxed);
        SLOT_TAIL_TICKS[slot].fetch_add(now.wrapping_sub(last_end), Ordering::Relaxed);
    }
    let absent = (members as u64).saturating_sub(present);
    ROUND_COUNT.fetch_add(1, Ordering::Relaxed);
    ROUND_WALL_TICKS.fetch_add(wall, Ordering::Relaxed);
    ROUND_ABSENT_TICKS.fetch_add(wall * absent, Ordering::Relaxed);
    ROUND_MEMBER_SLOTS.fetch_add(members as u64, Ordering::Relaxed);
}

/// One member's whole-forward totals, in the units each was recorded in.
#[derive(Debug, Clone, Copy)]
pub struct SlotRow {
    pub slot: usize,
    pub chunks: u64,
    pub busy_ticks: u64,
    pub cpu_nanos: u64,
    pub first_latency_ticks: u64,
    pub rounds: u64,
    pub tail_ticks: u64,
}

/// Round-level totals across the whole forward.
#[derive(Debug, Clone, Copy)]
pub struct RoundTotals {
    pub rounds: u64,
    pub wall_ticks: u64,
    pub absent_ticks: u64,
    pub member_slots: u64,
}

#[must_use]
pub fn slot_rows() -> Vec<SlotRow> {
    (0..MAX_SLOTS)
        .filter(|slot| SLOT_CHUNKS[*slot].load(Ordering::Relaxed) > 0)
        .map(|slot| SlotRow {
            slot,
            chunks: SLOT_CHUNKS[slot].load(Ordering::Relaxed),
            busy_ticks: SLOT_BUSY_TICKS[slot].load(Ordering::Relaxed),
            cpu_nanos: SLOT_CPU_NANOS[slot].load(Ordering::Relaxed),
            first_latency_ticks: SLOT_FIRST_LATENCY_TICKS[slot].load(Ordering::Relaxed),
            rounds: SLOT_ROUNDS[slot].load(Ordering::Relaxed),
            tail_ticks: SLOT_TAIL_TICKS[slot].load(Ordering::Relaxed),
        })
        .collect()
}

#[must_use]
pub fn round_totals() -> RoundTotals {
    RoundTotals {
        rounds: ROUND_COUNT.load(Ordering::Relaxed),
        wall_ticks: ROUND_WALL_TICKS.load(Ordering::Relaxed),
        absent_ticks: ROUND_ABSENT_TICKS.load(Ordering::Relaxed),
        member_slots: ROUND_MEMBER_SLOTS.load(Ordering::Relaxed),
    }
}

pub fn reset_round_attribution() {
    for slot in 0..MAX_SLOTS {
        SLOT_CHUNKS[slot].store(0, Ordering::Relaxed);
        SLOT_BUSY_TICKS[slot].store(0, Ordering::Relaxed);
        SLOT_CPU_NANOS[slot].store(0, Ordering::Relaxed);
        SLOT_FIRST_LATENCY_TICKS[slot].store(0, Ordering::Relaxed);
        SLOT_ROUNDS[slot].store(0, Ordering::Relaxed);
        SLOT_TAIL_TICKS[slot].store(0, Ordering::Relaxed);
    }
    ROUND_COUNT.store(0, Ordering::Relaxed);
    ROUND_WALL_TICKS.store(0, Ordering::Relaxed);
    ROUND_ABSENT_TICKS.store(0, Ordering::Relaxed);
    ROUND_MEMBER_SLOTS.store(0, Ordering::Relaxed);
}
