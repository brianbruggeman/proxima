...
printf '\n[1/8] sans-IO codec builds no_std + alloc (no default features)\n'
cargo build -p "${crate}" --no-default-features

printf '\n[2/8] crate builds clean with the client (all features)\n'
cargo build -p "${crate}" --all-features

printf '\n[3/8] codec + vendored corpus green (the RESP codec now lives in proxima-protocols::redis)\n'
cargo nextest run -p proxima-protocols --no-default-features --features redis --no-fail-fast

printf '\n[4/8] client facade tests green (ClientSession FSM + config + Pipe)\n'
cargo nextest run -p "${crate}" --features client --no-fail-fast

printf '\n[5/8] clippy pedantic clean across the feature matrix\n'
...
printf '\n[6/8] TOKIO GATE — the bare sans-IO codec graph must carry zero tokio\n'
...
printf '\n[7/8] bench scaffolding compiles (records data, does NOT seal)\n'
...
printf '\n[8/8] client facade doctests (features=client)\n'
