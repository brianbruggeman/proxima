...
declare -a COMBO_FEATURES=(
    ...
    "runtime-prime-executor,runtime-prime-inbox-alloc,runtime-prime-reactor,runtime-prime-bgpool"
)
...
#
# combos that pin `runtime-prime-inbox-alloc` report `n/a`: the two inbox
# layers are mutually exclusive by `compile_error!`, so no single bench
# spans every row. inbox-const is the widest-composing one we have.
BENCH_REQUIRED_FEATURE="runtime-prime-inbox-const"
...
`bench_inbox_const` bench (criterion, capped at warm-up 1s / measure 2s
