...
fn post_classify(addr: std::net::SocketAddr, body: &[u8]) -> (u16, String) {
    ...
    let (status, header_length, content_length) = loop {
        ...
        assert!(
            read > 0,
            "connection closed before a full response head arrived"
        );
    ...
    let body_text =
        String::from_utf8_lossy(&raw[header_length..header_length + content_length]).into_owned();
    (status, body_text)
}
...
fn parse_digit(body: &str) -> usize {
    ...
    body[start..end]
        .trim()
        .parse()
        .expect("digit field parses as an integer")
}
...
fn wire_to_weights_classifies_real_mnist_images_over_real_http() {
    ...
    let model = pipeline::load_model(model_path)
        .expect("load the real mnist.onnx checkpoint")
        .expect("checkpoint present, load_model returns Some");
    ...
    let handle = runtime
        ...
        .expect("serve_http binds through the prime AcceptorFactory");
    let addr = handle
        .bind_addr()
    ...
    assert!(
        bodies.len() >= SAMPLE_COUNT,
        "expected at least {SAMPLE_COUNT} real test images, got {}",
        bodies.len()
    );
    ...
    for (index, (body, &label)) in bodies.iter().zip(labels.iter()).enumerate() {
        ...
        assert_eq!(
            status, 200,
            "request {index} expected 200, got {status}: {response_body}"
        );
    ...
    eprintln!(
        "wire_to_weights: accuracy {accuracy:.4} ({correct}/{}) over real HTTP, average {average_latency:?}/request",
        bodies.len()
    );
    ...
    assert!(
        accuracy >= 0.90,
        "expected wire_to_weights to classify at least 90% of {} real test images over real HTTP, got {accuracy:.4}",
        bodies.len()
    );
}
