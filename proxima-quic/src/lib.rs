...
//!   the sans-IO `proxima_protocols::quic` state machine; **no quinn,
//!   no h3-quinn**. The TLS provider is caller-supplied via
//!   `proxima_protocols::quic::tls::TlsProvider` — this crate never
//!   picks one, so a consumer that wants the rustls bridge enables
//!   `proxima-protocols/quic-tls-rustls` and passes
//!   `quic::tls::rustls_provider`'s type in (that is what
//!   `proxima-http`'s http3 module does). Drivable by any executor:
//!   prime in production, tokio via `tokio-compat`, embassy, or a
//!   hand-rolled poll loop. Per workspace principle 5: no tokio in
//!   production builds.
//!
