//! `push_q4k_ggml_port_body`'s activation address now shares
//! `push_q4k_plain_product_y4_address` with `push_packed_row_blocked_body`'s
//! own `plain_product` arm, so it takes the SAME `other_stride_is_one`
//! stride-free specialization -- see that function's own doc. Before this
//! landing the port always multiplied by `other_stride` at read time, which
//! happens to be correct at every stride (multiplying by 1 is a no-op), so
//! no existing `metal-q4k-ggml-port` test (`q4k_ggml_port_parity.rs`, always
//! a contiguous `[k, 1]` activation) could tell the two code paths apart.
//! This file drives BOTH branches of `other_stride_is_one` through the port
//! body directly: a unit-stride activation (the specialization's new path)
//! and a `NaN`-poisoned non-unit-stride activation (`q6k_matmul_strided_
//! activation.rs`'s own technique, one codec over) -- a wrong-stride read on
//! either branch lands on a poisoned slot or disagrees with the independent
//! dequantize+dot reference.
...
/// shape, `Q4_K` instead of `Q6_K`) with the activation read through a
/// reduce-axis stride of `stride` rather than a plain projection --
/// `stride == 1` degenerates to a plain contiguous read (`other_stride_is_one`
/// true), any larger value forces the generic multiply-by-`other_stride` arm.
    ...
    for (index, (&metal_value, &reference)) in metal_root.iter().zip(expected.iter()).enumerate() {
        assert!(
            ...
            "{label} row {index}: metal={metal_value} is not finite -- a wrong-stride read landed \
             on a poisoned NaN activation slot in `push_q4k_ggml_port_body`'s stride-{stride} arm"
        );
        ...
        assert!(
            ...
            "{label} row {index}: metal={metal_value} disagrees with the independent \
             ...
             `push_q4k_ggml_port_body`'s stride-{stride} activation addressing defect if it fires"
        );
    }
}
