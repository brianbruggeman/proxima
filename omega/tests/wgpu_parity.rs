...
use proxima_tensor::{
    ...
    Reduce, ReduceInit, ScalarOp, TensorError, append, map, projection,
};
...
fn the_two_layer_mlp_runs_on_wgpu_at_cpu_parity() {
    ...
    for (&got, &want) in actual.iter().zip(expected.iter()) {
        assert!(
            got.is_finite(),
            "wgpu, via the wrapper, produced a non-finite value: {got}"
        );
    ...
    eprintln!(
        "wgpu mlp parity: max_diff={max_diff} max_magnitude={max_magnitude} relative={relative}"
    );
    assert!(
        relative < 1e-4,
...
fn f16_matmul_runs_on_wgpu_within_the_metal_parity_f16_epsilon_or_names_its_rejection() {
    ...
    let named: Vec<(&str, QuantizedBlock<'_>)> = vec![
        ("lhs", QuantizedBlock::Float32(&lhs)),
    ...
    match execute_plan_named(&mut wgpu_plan, &named) {
        Ok(wgpu) => {
            ...
            for (&got, &want) in actual.iter().zip(expected.iter()) {
                assert!(
                    got.is_finite(),
                    "wgpu f16 matmul produced a non-finite value: {got}"
                );
            ...
            eprintln!(
                "wgpu f16 parity: max_relative_diff={max_diff} max_absolute_diff={max_absolute}"
            );
            for (&got, &want) in actual.iter().zip(expected.iter()) {
                let absolute = (got - want).abs();
...
fn assert_packed_codec_parity(
    ...
    let activation: Vec<f32> = random_vec(31, k as usize)
        .into_iter()
        ...
        .zip(weight_blocks.chunks_exact_mut(blocks_per_row * block_bytes))
    {
        quantize(row_f32, row_blocks)
            .expect("row length is a whole multiple of the codec's block width");
    }
    ...
    let f32_named: Vec<(&str, QuantizedBlock<'_>)> = vec![
    ...
    for (&got, &want) in actual.iter().zip(expected.iter()) {
        assert!(
            got.is_finite(),
            "wgpu {codec_name} matmul produced a non-finite value: {got}"
        );
        max_diff = max_diff.max((got - want).abs());
    }
    let max_magnitude = expected
    ...
    let relative = max_diff / max_magnitude.max(f32::MIN_POSITIVE);
    eprintln!(
        "wgpu packed-{codec_name} parity: rows={rows} k={k} max_diff={max_diff} relative={relative}"
    );
    assert!(
        relative < tolerance,
...
fn matmul_runs_on_wgpu_at_cpu_parity_whichever_reduce_path_the_adapter_takes() {
    ...
    for (&got, &want) in actual.iter().zip(expected.iter()) {
        assert!(
            got.is_finite(),
            "wgpu matmul ({path}) produced a non-finite value: {got}"
        );
        max_diff = max_diff.max((got - want).abs());
    }
    let max_magnitude = expected
...
fn the_full_mistral_cached_forward_runs_on_wgpu_at_cpu_parity() {
    ...
    for (&got, &want) in actual.iter().zip(expected.iter()) {
        assert!(
            got.is_finite(),
            "wgpu, via the wrapper, produced a non-finite logit: {got}"
        );
        max_diff = max_diff.max((got - want).abs());
    }
    let max_magnitude = expected
