//! Device parity for `Q6_K`, on the REAL bytes `output.weight` carries in
//! `openchat-3.5-1210.Q4_K_S.gguf` -- not synthetic, per guiding-principles
//! §9. `output.weight` is the LONE `Q6_K` tensor in this checkpoint
//! (`proxima-tensor/docs/discipline.md`'s own inventory: `Q4_K x217, F32
//! x65, Q5_K x8, Q6_K x1`) and measured ~150ms of ~250.7ms total `gpu_exec`
//! (roughly 60%) before this landing, because the loader dequantized it to
//! 524 MB of f32 before Metal ever saw it. This is the shape this landing
//! exists for.
//!
//! Skips (does not fail) when the real file is not present on this host --
//! matching `proxima-tensor/src/cpu.rs`'s own `REAL_OPENCHAT_GGUF_PATH`
//! tests, which this file's fixture-reading code mirrors (that helper is
...
/// `metal_parity.rs`'s `q4k_matmul_program` builds, restated here since this
/// is a standalone integration test binary.
fn matmul_program(rows: u32, k: u32, weight_dtype: DType) -> (Vec<Op>, NodeId) {
    ...
    let sum = append(
        ...
        Op::Reduce(Reduce {
            ...
            keep: Keep::Reduce,
            name: Some("q6k_real_matmul".into()),
        }),
    );
    (program, sum)
}

/// `output.weight` is 4096 x 32002 (`embedding x vocab`) in this checkpoint;
/// running the full 32002 rows on a device parity test is unnecessary to
...
/// exactly a row-count prefix).
const ROWS_TO_CHECK: usize = 64;

#[test]
fn metal_matmul_on_real_output_weight_q6k_bytes_matches_the_dequantized_f32_cpu_path() {
    ...
    let Some((weight_bytes, in_dim, out_dim)) =
        real_tensor_bytes(&mut file, &parsed, file_len, "output.weight", GgmlType::Q6_K)
    else {
        return;
    };

    let blocks_per_row = in_dim / q6_k::QK_K;
    assert_eq!(blocks_per_row * q6_k::QK_K, in_dim, "output.weight's in_dim is a whole number of Q6_K super-blocks");
    let row_bytes = blocks_per_row * q6_k::BLOCK_BYTES;
    assert_eq!(weight_bytes.len(), row_bytes * out_dim, "output.weight byte length matches its declared shape");
    ...
    for (row_blocks, row_f32) in sliced_weight.chunks_exact(row_bytes).zip(dequantized.chunks_exact_mut(in_dim)) {
        q6_k::dequantize(row_blocks, row_f32).expect("a whole number of q6_k super-blocks per row");
    }
    ...
    let metal = omega::execute(
        &packed_program,
        &[],
        &[QuantizedBlock::Q6K(sliced_weight), QuantizedBlock::Float32(&activation)],
        &[packed_sum],
    )
    .expect("metal executes a packed q6_k matmul on real output.weight bytes");
    ...
    eprintln!(
        "real output.weight (Q6_K, {rows} of {out_dim} rows, k={in_dim}) metal vs dequantized-f32 cpu: \
