...
fn run_real_ffn_down_parity(relative_tolerance: f32) {
    ...
    assert_eq!(
        blocks_per_row * q5_k::QK_K,
        in_dim,
        "blk.0.ffn_down.weight's in_dim is a whole number of Q5_K super-blocks"
    );
    let row_bytes = blocks_per_row * q5_k::BLOCK_BYTES;
    assert_eq!(
        weight_bytes.len(),
        row_bytes * out_dim,
        "blk.0.ffn_down.weight byte length matches its declared shape"
    );
    ...
    let metal = omega::execute(
        ...
        &[],
        &[
            QuantizedBlock::Packed { codec: Codec::Q5K, bytes: sliced_weight },
    ...
    assert!(
        ...
        "packed unpack disagrees with the dequantized reference on REAL checkpoint bytes: \
         relative={relative} max_diff={max_diff} tolerance={relative_tolerance}"
    );
}
