...
mod support;
use support::{as_named_blocks, qwen3_gqa_qk_norm_forward_fixture};

/// One `(cached_len, new_count)` cell: binds the qwen3 GQA+QK-norm fixture,
/// asserts it actually fuses into `BoundOpKind::CachedAttention` (a
...
fn parity_cell(cached_len: u64, new_count: u64) -> f32 {
...
fn the_two_range_cached_attention_kernel_holds_parity_on_the_qwen3_gqa_qk_norm_shape() {
    ...
    for cached_len in [0u64, 5, 33] {
        for new_count in [1u64, 4] {
            let max_diff = parity_cell(cached_len, new_count);
