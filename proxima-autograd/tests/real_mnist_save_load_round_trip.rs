...
fn evaluate_accuracy(test_images: &[f32], test_labels: &[u8], w1: &[f32], b1: &[f32], w2: &[f32], b2: &[f32]) -> f64 {
    ...
    let eval_named: Vec<(&str, &[f32])> = alloc::vec![("x", test_images), ("w1", w1), ("b1", b1), ("w2", w2), ("b2", b2)];
    let evaluated = proxima_tensor::cpu::evaluate_named(&eval_program, &[], &eval_named, &[eval_logits]).expect("evaluate the mlp on real held-out mnist test images");
...
fn trained_mlp_survives_a_save_load_round_trip_at_identical_accuracy() {
    ...
    save_state(&program, &final_state, checkpoint.path()).expect("save_state writes the trained checkpoint");
    ...
    let pre_save_accuracy = evaluate_accuracy(&test_images, &test_labels, final_w1, final_b1, final_w2, final_b2);
    std::eprintln!("pre-save accuracy: {pre_save_accuracy:.6}");
    assert!(pre_save_accuracy > 0.0, "sanity: the trained network must classify at least one test digit correctly");
    ...
    leaf(&mut load_program, "w1", alloc::vec![Extent::Static(IN_DIM as u32), Extent::Static(HIDDEN_DIM as u32)]);
    leaf(&mut load_program, "b1", alloc::vec![Extent::Static(HIDDEN_DIM as u32)]);
    leaf(&mut load_program, "w2", alloc::vec![Extent::Static(HIDDEN_DIM as u32), Extent::Static(OUT_DIM as u32)]);
    leaf(&mut load_program, "b2", alloc::vec![Extent::Static(OUT_DIM as u32)]);
    leaf(&mut load_program, "m_w1", alloc::vec![Extent::Static(IN_DIM as u32), Extent::Static(HIDDEN_DIM as u32)]);
    leaf(&mut load_program, "v_w1", alloc::vec![Extent::Static(IN_DIM as u32), Extent::Static(HIDDEN_DIM as u32)]);
    leaf(&mut load_program, "m_b1", alloc::vec![Extent::Static(HIDDEN_DIM as u32)]);
    leaf(&mut load_program, "v_b1", alloc::vec![Extent::Static(HIDDEN_DIM as u32)]);
    leaf(&mut load_program, "m_w2", alloc::vec![Extent::Static(HIDDEN_DIM as u32), Extent::Static(OUT_DIM as u32)]);
    leaf(&mut load_program, "v_w2", alloc::vec![Extent::Static(HIDDEN_DIM as u32), Extent::Static(OUT_DIM as u32)]);
    leaf(&mut load_program, "m_b2", alloc::vec![Extent::Static(OUT_DIM as u32)]);
    leaf(&mut load_program, "v_b2", alloc::vec![Extent::Static(OUT_DIM as u32)]);

    let loaded_state = load_state(&load_program, checkpoint.path()).expect("load_state reads the checkpoint back");
    assert_eq!(loaded_state.len(), final_state.len(), "every rebind name must round-trip through the checkpoint");

    let loaded_w1 = &loaded_state.iter().find(|(name, _)| name == "w1").expect("loaded w1 present").1;
    let loaded_b1 = &loaded_state.iter().find(|(name, _)| name == "b1").expect("loaded b1 present").1;
    let loaded_w2 = &loaded_state.iter().find(|(name, _)| name == "w2").expect("loaded w2 present").1;
    let loaded_b2 = &loaded_state.iter().find(|(name, _)| name == "b2").expect("loaded b2 present").1;

    assert_eq!(loaded_w1, final_w1, "w1 must round-trip bit-identical through save_state/load_state");
    assert_eq!(loaded_b1, final_b1, "b1 must round-trip bit-identical through save_state/load_state");
    assert_eq!(loaded_w2, final_w2, "w2 must round-trip bit-identical through save_state/load_state");
    assert_eq!(loaded_b2, final_b2, "b2 must round-trip bit-identical through save_state/load_state");

    let post_load_accuracy = evaluate_accuracy(&test_images, &test_labels, loaded_w1, loaded_b1, loaded_w2, loaded_b2);
    std::eprintln!("post-load accuracy: {post_load_accuracy:.6}");
    assert_eq!(post_load_accuracy, pre_save_accuracy, "accuracy must be identical before save and after a fresh-session load");
    ...
    let manifest = proxima_safetensors::SafetensorsParser::new().push(&written_bytes).expect("parser accepts the written checkpoint").finish().expect("checkpoint parses as a well-formed safetensors manifest");
    ...
    for (name, shape) in [
        ...
        let entry = manifest.tensor(name).unwrap_or_else(|| panic!("{name} present in the parsed manifest"));
        assert_eq!(entry.dtype, proxima_tensor::DType::Float32, "{name} dtype must round-trip as Float32");
