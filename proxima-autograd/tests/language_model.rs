    ...
    let compact_ids = real_ids
        ...
        .map(|&real_id| {
            compact_to_real
                ...
                .expect("every real id in the corpus is present in its own restricted vocab") as u32
        })
        .collect();
...
fn build_language_model(vocab_size: usize) -> Network {
    ...
    let q_product = elementwise(&mut program, ScalarOp::Multiply, alloc::vec![(x, proj(4, &[0, 3])), (params.wq, proj(4, &[3, 1, 2]))]);
    let q = reduce_add(&mut program, q_product, identity(4), proj(4, &[0, 1, 2]));
    let k_product = elementwise(&mut program, ScalarOp::Multiply, alloc::vec![(x, proj(4, &[0, 3])), (params.wk, proj(4, &[3, 1, 2]))]);
    let k = reduce_add(&mut program, k_product, identity(4), proj(4, &[0, 1, 2]));
    let v_product = elementwise(&mut program, ScalarOp::Multiply, alloc::vec![(x, proj(4, &[0, 3])), (params.wv, proj(4, &[3, 1, 2]))]);
    ...
    let attn_product = elementwise(&mut program, ScalarOp::Multiply, alloc::vec![(attended, proj(4, &[0, 1, 2])), (params.wo, proj(4, &[1, 2, 3]))]);
    ...
    let gate_product = elementwise(&mut program, ScalarOp::Multiply, alloc::vec![(residual1, proj(3, &[0, 1])), (params.w1, proj(3, &[1, 2]))]);
    let gate = reduce_add(&mut program, gate_product, identity(3), proj(3, &[0, 2]));
    let gate_biased = elementwise(&mut program, ScalarOp::Add, alloc::vec![(gate, identity(2)), (params.b1, proj(2, &[1]))]);
    let hidden = relu(&mut program, DType::Float32, gate_biased, 2);

    let down_product = elementwise(&mut program, ScalarOp::Multiply, alloc::vec![(hidden, proj(3, &[0, 1])), (params.w2, proj(3, &[1, 2]))]);
    let ffn_out = reduce_add(&mut program, down_product, identity(3), proj(3, &[0, 2]));
    let ffn_out_biased = elementwise(&mut program, ScalarOp::Add, alloc::vec![(ffn_out, identity(2)), (params.b2, proj(2, &[1]))]);
    let residual2 = elementwise(&mut program, ScalarOp::Add, alloc::vec![(residual1, identity(2)), (ffn_out_biased, identity(2))]);
    ...
    let logits_product = elementwise(&mut program, ScalarOp::Multiply, alloc::vec![(residual2, proj(3, &[0, 1])), (params.table, proj(3, &[2, 1]))]);
    ...
    let probabilities_lm_stabilized = elementwise(&mut program, ScalarOp::Add, alloc::vec![(probabilities_lm, identity(2)), (log_epsilon, broadcast(2))]);
    let log_probabilities = elementwise(&mut program, ScalarOp::Logarithm, alloc::vec![(probabilities_lm_stabilized, identity(2))]);
    let weighted = elementwise(&mut program, ScalarOp::Multiply, alloc::vec![(onehot, identity(2)), (log_probabilities, identity(2))]);
    let per_position_loss = reduce_add(&mut program, weighted, identity(2), proj(2, &[0]));
    let negated = elementwise(&mut program, ScalarOp::Negate, alloc::vec![(per_position_loss, identity(1))]);
    let total_sum = reduce_add(&mut program, negated, identity(1), proj(1, &[]));
    let inv_seq_len = constant(&mut program, 1.0 / SEQ_LEN as f32);
    let loss = elementwise(&mut program, ScalarOp::Multiply, alloc::vec![(total_sum, identity(0)), (inv_seq_len, broadcast(0))]);

    Network { program, params, logits, loss }
}
...
fn gradient_check_tensor(
    ...
    for index in checked_indices(analytic.len()) {
        ...
        if !within_tolerance(analytic[index], numeric) {
            let is_worse_violation = violation.is_none_or(|(_, previous_analytic, previous_numeric): (usize, f32, f32)| {
    ...
    GradientCheckReport { worst_relative: worst.0, worst_relative_index: worst.1, violation }
}
...
async fn whole_model_gradient_check_matches_central_difference_on_every_tensor() {
    ...
    let grad_table_dense = differentiated.gradient_of_named("table").expect("the tied LM head reads table densely");
    let table_gathered: alloc::vec::Vec<_> = differentiated.gathered_gradients_of_named("table").collect();
    assert_eq!(table_gathered.len(), 1, "table is gathered by exactly the one input-embedding site");
    ...
    let evaluated = evaluate_named(
        ...
        &[grad_wq, grad_wk, grad_wv, grad_wo, grad_w1, grad_b1, grad_w2, grad_b2, grad_table_dense, table_gathered.values],
    )
    ...
    let analytic_table = combine_table_gradient(dense_table, &unique_ids, &summed, corpus.vocab_size, D_MODEL);

    let step = 1e-3f32;
    let report: alloc::vec::Vec<(&str, GradientCheckReport)> = alloc::vec![
        ("wq", gradient_check_tensor(&network.program, network.loss, &ids, &onehot, &mut weights, |weights| &mut weights.wq, &analytic_wq, step)),
        ("wk", gradient_check_tensor(&network.program, network.loss, &ids, &onehot, &mut weights, |weights| &mut weights.wk, &analytic_wk, step)),
        ("wv", gradient_check_tensor(&network.program, network.loss, &ids, &onehot, &mut weights, |weights| &mut weights.wv, &analytic_wv, step)),
        ("wo", gradient_check_tensor(&network.program, network.loss, &ids, &onehot, &mut weights, |weights| &mut weights.wo, &analytic_wo, step)),
        ("w1", gradient_check_tensor(&network.program, network.loss, &ids, &onehot, &mut weights, |weights| &mut weights.w1, &analytic_w1, step)),
        ("b1", gradient_check_tensor(&network.program, network.loss, &ids, &onehot, &mut weights, |weights| &mut weights.b1, &analytic_b1, step)),
        ("w2", gradient_check_tensor(&network.program, network.loss, &ids, &onehot, &mut weights, |weights| &mut weights.w2, &analytic_w2, step)),
        ("b2", gradient_check_tensor(&network.program, network.loss, &ids, &onehot, &mut weights, |weights| &mut weights.b2, &analytic_b2, step)),
        ("table (dense + gathered combined)", gradient_check_tensor(&network.program, network.loss, &ids, &onehot, &mut weights, |weights| &mut weights.table, &analytic_table, step)),
    ];
    ...
    for (name, result) in &report {
        std::eprintln!(
            ...
            result.worst_relative, result.worst_relative_index, result.violation
        );
    }
    for (name, result) in &report {
        assert!(
            ...
            result.violation, result.worst_relative
        );
    }
}
...
async fn adam_training_overfits_the_real_corpus_toward_near_zero_loss() {
    ...
    let grad_b2 = differentiated.gradient_of_named("b2").expect("b2 feeds the loss");
    let grad_table_dense = differentiated.gradient_of_named("table").expect("the tied LM head reads table densely");
    let table_gathered = differentiated.gathered_gradients_of_named("table").next().expect("table is gathered once");
    ...
    let wq_nodes = append_adam(&mut program, &config, 3, &qkv_extents, "wq", network.params.wq, grad_wq, step_node);
    let wk_nodes = append_adam(&mut program, &config, 3, &qkv_extents, "wk", network.params.wk, grad_wk, step_node);
    let wv_nodes = append_adam(&mut program, &config, 3, &qkv_extents, "wv", network.params.wv, grad_wv, step_node);
    let wo_nodes = append_adam(&mut program, &config, 3, &wo_extents, "wo", network.params.wo, grad_wo, step_node);
    let w1_nodes = append_adam(&mut program, &config, 2, &[D_MODEL as u32, FFN_HIDDEN as u32], "w1", network.params.w1, grad_w1, step_node);
    let b1_nodes = append_adam(&mut program, &config, 1, &[FFN_HIDDEN as u32], "b1", network.params.b1, grad_b1, step_node);
    let w2_nodes = append_adam(&mut program, &config, 2, &[FFN_HIDDEN as u32, D_MODEL as u32], "w2", network.params.w2, grad_w2, step_node);
    let b2_nodes = append_adam(&mut program, &config, 1, &[D_MODEL as u32], "b2", network.params.b2, grad_b2, step_node);
    ...
    let table_param = leaf(&mut table_program, "table", alloc::vec![Extent::Static(corpus.vocab_size as u32), Extent::Static(D_MODEL as u32)]);
    let table_grad_in = leaf(&mut table_program, "table_grad", alloc::vec![Extent::Static(corpus.vocab_size as u32), Extent::Static(D_MODEL as u32)]);
    let table_step = step_input(&mut table_program, "step");
    let table_adam = append_adam(&mut table_program, &config, 2, &[corpus.vocab_size as u32, D_MODEL as u32], "table", table_param, table_grad_in, table_step);
    ...
    for step in 0..STEPS {
        ...
        let evaluated = evaluate_named(
            ...
            &[
                network.loss,
                grad_table_dense, table_gathered.values,
        ...
        let dense_table = evaluated.get(grad_table_dense).expect("requested").0;
        let gathered_values = evaluated.get(table_gathered.values).expect("requested").0;
        let (unique_ids, summed) = sparse::dedupe_and_sum_rows(&ids, gathered_values, D_MODEL).expect("ids/contribution line up");
        let combined_table_grad = combine_table_gradient(dense_table, &unique_ids, &summed, corpus.vocab_size, D_MODEL);
        ...
        weights.wq = evaluated.get(wq_nodes.new_param).expect("requested").0.to_vec();
        ...
        weights.wk = evaluated.get(wk_nodes.new_param).expect("requested").0.to_vec();
        ...
        weights.wv = evaluated.get(wv_nodes.new_param).expect("requested").0.to_vec();
        ...
        weights.wo = evaluated.get(wo_nodes.new_param).expect("requested").0.to_vec();
        ...
        weights.w1 = evaluated.get(w1_nodes.new_param).expect("requested").0.to_vec();
        ...
        weights.b1 = evaluated.get(b1_nodes.new_param).expect("requested").0.to_vec();
        ...
        weights.w2 = evaluated.get(w2_nodes.new_param).expect("requested").0.to_vec();
        ...
        weights.b2 = evaluated.get(b2_nodes.new_param).expect("requested").0.to_vec();
    ...
    assert!(final_loss < 0.05, "expected near-zero overfit loss on {STEPS} steps, got {final_loss} (started at {initial})");
