...
fn build_head_and_loss(
    ...
    let logits_product = elementwise(
        program,
        ScalarOp::Multiply,
        alloc::vec![(residual2, proj(3, &[0, 1])), (table, proj(3, &[2, 1]))],
    );
...
fn build_dense_ffn(program: &mut Vec<Op>, residual1: NodeId, ffn: &DenseFfnParams) -> NodeId {
    let gate_product = elementwise(
        program,
        ScalarOp::Multiply,
        alloc::vec![(residual1, proj(3, &[0, 1])), (ffn.w1, proj(3, &[1, 2]))],
    );
    let gate = reduce_add(program, gate_product, identity(3), proj(3, &[0, 2]));
    let gate_biased = elementwise(
        program,
        ScalarOp::Add,
        alloc::vec![(gate, identity(2)), (ffn.b1, proj(2, &[1]))],
    );
...
fn declare_sparse_ffn(program: &mut Vec<Op>, retained_bands: &[usize]) -> SparseFfnParams {
    let bands = retained_bands
        ...
        .map(|&band| {
            let w1_band = leaf(
                program,
                &alloc::format!("w1_band{band}"),
                alloc::vec![
                    Extent::Static(D_MODEL as u32),
                    Extent::Static(BAND_WIDTH as u32)
                ],
            );
            let b1_band = leaf(
                program,
                &alloc::format!("b1_band{band}"),
                alloc::vec![Extent::Static(BAND_WIDTH as u32)],
            );
            let w2_band = leaf(
                program,
                &alloc::format!("w2_band{band}"),
                alloc::vec![
                    Extent::Static(BAND_WIDTH as u32),
                    Extent::Static(D_MODEL as u32)
                ],
...
fn build_sparse_ffn(program: &mut Vec<Op>, residual1: NodeId, sparse: &SparseFfnParams) -> NodeId {
    ...
    for &(_band, w1_band, b1_band, w2_band) in &sparse.bands {
        let gate_product = elementwise(
            program,
            ScalarOp::Multiply,
            alloc::vec![(residual1, proj(3, &[0, 1])), (w1_band, proj(3, &[1, 2]))],
        );
        let gate = reduce_add(program, gate_product, identity(3), proj(3, &[0, 2]));
        let gate_biased = elementwise(
            program,
            ScalarOp::Add,
            alloc::vec![(gate, identity(2)), (b1_band, proj(2, &[1]))],
        );
        ...
        let down_product = elementwise(
            program,
            ScalarOp::Multiply,
            alloc::vec![(hidden_band, proj(3, &[0, 1])), (w2_band, proj(3, &[1, 2]))],
        );
    ...
    for &partial in &partials[1..] {
        ffn_out = elementwise(
            program,
            ScalarOp::Add,
            alloc::vec![(ffn_out, identity(2)), (partial, identity(2))],
        );
    }
    let ffn_out_biased = elementwise(
        program,
        ScalarOp::Add,
        alloc::vec![(ffn_out, identity(2)), (sparse.b2, proj(2, &[1]))],
    );
    elementwise(
        program,
        ScalarOp::Add,
...
fn greedy_sample_matches(
    ...
    while generated.len() < SEQ_LEN {
        ...
        let evaluated = evaluate_named(program, &[], &named, &[logits])
...
async fn structural_counts_dense_ffn_vs_two_of_four_bands_retained() {
    ...
    let residual1_stand_in_sparse = leaf(
        &mut sparse_program,
        "residual1",
        alloc::vec![
            Extent::Static(SEQ_LEN as u32),
    ...
    assert_eq!(
        dense_macs,
        (SEQ_LEN * D_MODEL * FFN_HIDDEN * 2) as u64,
        "dense FFN macs = seq*d_model*hidden for w1 + same for w2"
    );
    assert_eq!(
        sparse_macs,
        (SEQ_LEN * D_MODEL * BAND_WIDTH * 2 * 2) as u64,
        "2 retained bands each cost seq*d_model*band_width for w1 and w2"
    );
    assert!(
        sparse_macs < dense_macs,
        "retaining half the bands must cost fewer macs than dense, not the same or more"
    );
}
...
async fn pruned_ffn_bands_degrade_loss_then_recover_with_fine_tuning() {
    ...
    let (_dense_logits, dense_loss) = build_head_and_loss(
        &mut dense_program,
        residual2,
        attn_params.table,
        onehot_node,
    ...
    let differentiated =
        differentiate(&dense_program, dense_loss).expect("scalar loss differentiates");
    let grad_table = differentiated
        .gradient_of_named("table")
        .expect("table feeds the loss");
    ...
    for step in 0..DENSE_STEPS {
        ...
        attn.table = evaluated
            .get(table_nodes.new_param)
            .expect("requested")
            .0
            .to_vec();
        table_state.m = evaluated
            .get(table_nodes.new_m)
            .expect("requested")
            .0
            .to_vec();
        table_state.v = evaluated
            .get(table_nodes.new_v)
            .expect("requested")
            .0
            .to_vec();
        attn.wq = evaluated
    ...
    std::eprintln!(
        "dense phase: initial loss {dense_initial_loss}, final loss {dense_final_loss} over {DENSE_STEPS} steps"
    );
    assert!(
        dense_loss_curve.iter().all(|value| value.is_finite()),
        "dense loss went non-finite"
    );
    assert!(
        dense_final_loss < 0.05,
        "dense baseline must overfit near-zero before pruning is a meaningful comparison, got {dense_final_loss}"
    );

    // ---- Phase 2: rank bands by magnitude, drop the 2 lowest. ----
    let mut magnitudes: alloc::vec::Vec<(usize, f32)> = (0..BAND_COUNT)
        .map(|band| (band, band_magnitude(&ffn.w1, &ffn.w2, band)))
        .collect();
    magnitudes.sort_by(|a, b| a.1.partial_cmp(&b.1).expect("magnitudes are finite"));
    std::eprintln!("band magnitudes (ascending): {magnitudes:?}");
    let mut retained_bands: alloc::vec::Vec<usize> =
        magnitudes[2..].iter().map(|&(band, _)| band).collect();
    ...
    let (_sparse_x, sparse_residual1) =
        build_attention(&mut sparse_program, &sparse_attn_params, sparse_ids_node);
    let sparse_residual2 =
        build_sparse_ffn(&mut sparse_program, sparse_residual1, &sparse_ffn_params);
    let (sparse_logits, sparse_loss) = build_head_and_loss(
        &mut sparse_program,
        sparse_residual2,
        sparse_attn_params.table,
        sparse_onehot_node,
    );

    let ffn_named_from_trained: alloc::vec::Vec<(alloc::string::String, alloc::vec::Vec<f32>)> =
        retained_bands
            .iter()
            .flat_map(|&band| {
                alloc::vec![
                    (
                        alloc::format!("w1_band{band}"),
                        w1_band_slice(&ffn.w1, band)
                    ),
                    (
                        alloc::format!("b1_band{band}"),
                        b1_band_slice(&ffn.b1, band)
                    ),
                    (
                        alloc::format!("w2_band{band}"),
                        w2_band_slice(&ffn.w2, band)
                    ),
                ]
            })
            .collect();
    ...
    let loss_immediately_after_pruning = evaluated_at_prune
        .get(sparse_loss)
    ...
    let sparse_differentiated =
        differentiate(&sparse_program, sparse_loss).expect("sparse scalar loss differentiates");
    let sparse_grad_w1_band0 = sparse_differentiated
        .gradient_of_named(&alloc::format!("w1_band{}", retained_bands[0]))
        .expect("retained band feeds the loss");
    let sparse_grad_w2_band0 = sparse_differentiated
        .gradient_of_named(&alloc::format!("w2_band{}", retained_bands[0]))
        .expect("retained band feeds the loss");
    let check_named: alloc::vec::Vec<(&str, &[f32])> = named_at_prune.clone();
    let evaluated_grads = evaluate_named(
        &sparse_differentiated.program,
        &[],
        &check_named,
        &[sparse_grad_w1_band0, sparse_grad_w2_band0],
    )
    .expect("sparse adjoint program lowers and evaluates");
    let analytic_w1_band0 = evaluated_grads
        .get(sparse_grad_w1_band0)
        .expect("requested")
        .0
        .to_vec();
    let analytic_w2_band0 = evaluated_grads
        .get(sparse_grad_w2_band0)
        .expect("requested")
        .0
        .to_vec();
    ...
    std::eprintln!(
        "sparse-graph gradient check on the retained band closest to band 0: violation={worst_violation:?}"
    );
    assert!(
        worst_violation.is_none(),
        "sparse graph's adjoint disagreed with central difference: {worst_violation:?}"
    );

    // ---- Phase 5: fine-tune the sparse graph. ----
    let sparse_grad_table = sparse_differentiated
        .gradient_of_named("table")
        .expect("table feeds the loss");
    let sparse_grad_wq = sparse_differentiated
        .gradient_of_named("wq")
        .expect("wq feeds the loss");
    let sparse_grad_wk = sparse_differentiated
        .gradient_of_named("wk")
        .expect("wk feeds the loss");
    let sparse_grad_wv = sparse_differentiated
        .gradient_of_named("wv")
        .expect("wv feeds the loss");
    let sparse_grad_wo = sparse_differentiated
        .gradient_of_named("wo")
        .expect("wo feeds the loss");
    let sparse_grad_b2 = sparse_differentiated
        .gradient_of_named("b2")
        .expect("b2 feeds the loss");
    let mut band_grads: alloc::vec::Vec<(usize, NodeId, NodeId, NodeId)> = alloc::vec::Vec::new();
    for &band in &retained_bands {
        let grad_w1_band = sparse_differentiated
            .gradient_of_named(&alloc::format!("w1_band{band}"))
            .expect("band feeds the loss");
        let grad_b1_band = sparse_differentiated
            .gradient_of_named(&alloc::format!("b1_band{band}"))
            .expect("band feeds the loss");
        let grad_w2_band = sparse_differentiated
            .gradient_of_named(&alloc::format!("w2_band{band}"))
            .expect("band feeds the loss");
    ...
    let sparse_table_nodes = append_adam(
        &mut sparse_train_program,
        &config,
        2,
        &[corpus.vocab_size as u32, D_MODEL as u32],
        "s_table",
    ...
    let sparse_wq_nodes = append_adam(
        &mut sparse_train_program,
        &config,
        3,
        &qkv_extents,
        "s_wq",
        sparse_attn_params.wq,
        sparse_grad_wq,
        sparse_step_node,
    );
    let sparse_wk_nodes = append_adam(
        &mut sparse_train_program,
        &config,
        3,
        &qkv_extents,
        "s_wk",
        sparse_attn_params.wk,
        sparse_grad_wk,
        sparse_step_node,
    );
    let sparse_wv_nodes = append_adam(
        &mut sparse_train_program,
        &config,
        3,
        &qkv_extents,
        "s_wv",
        sparse_attn_params.wv,
        sparse_grad_wv,
        sparse_step_node,
    );
    let sparse_wo_nodes = append_adam(
        &mut sparse_train_program,
        &config,
        3,
        &wo_extents,
        "s_wo",
        sparse_attn_params.wo,
        sparse_grad_wo,
        sparse_step_node,
    );
    let sparse_b2_nodes = append_adam(
        &mut sparse_train_program,
        &config,
        1,
        &[D_MODEL as u32],
        "s_b2",
        sparse_ffn_params.b2,
        sparse_grad_b2,
        sparse_step_node,
    );
    let mut sparse_band_nodes: alloc::vec::Vec<(usize, AdamNodes, AdamNodes, AdamNodes)> =
        alloc::vec::Vec::new();
    for &(band, grad_w1_band, grad_b1_band, grad_w2_band) in &band_grads {
        let (_band_check, w1_leaf, b1_leaf, w2_leaf) = *sparse_ffn_params
            .bands
            .iter()
            .find(|(candidate, ..)| *candidate == band)
            .expect("band declared");
        let w1_adam = append_adam(
            &mut sparse_train_program,
            &config,
            2,
            &[D_MODEL as u32, BAND_WIDTH as u32],
            &alloc::format!("s_w1_band{band}"),
            w1_leaf,
            grad_w1_band,
            sparse_step_node,
        );
        let b1_adam = append_adam(
            &mut sparse_train_program,
            &config,
            1,
            &[BAND_WIDTH as u32],
            &alloc::format!("s_b1_band{band}"),
            b1_leaf,
            grad_b1_band,
            sparse_step_node,
        );
        let w2_adam = append_adam(
            &mut sparse_train_program,
            &config,
            2,
            &[BAND_WIDTH as u32, D_MODEL as u32],
            &alloc::format!("s_w2_band{band}"),
            w2_leaf,
            grad_w2_band,
            sparse_step_node,
        );
    ...
    let mut sparse_attn = AttnWeights {
        table: attn.table.clone(),
        wq: attn.wq.clone(),
        wk: attn.wk.clone(),
        wv: attn.wv.clone(),
        wo: attn.wo.clone(),
    ...
    let mut band_states: alloc::vec::Vec<(usize, AdamState, AdamState, AdamState)> = retained_bands
        ...
        .map(|&band| {
            (
                band,
                zero_state(D_MODEL * BAND_WIDTH),
                zero_state(BAND_WIDTH),
                zero_state(BAND_WIDTH * D_MODEL),
            )
        })
        .collect();
    ...
    for step in 0..FINE_TUNE_STEPS {
        ...
        let mut named: alloc::vec::Vec<(&str, &[f32])> = alloc::vec![
            ("ids", ids.as_slice()),
            ("onehot", onehot.as_slice()),
            ("table", sparse_attn.table.as_slice()),
            ("wq", sparse_attn.wq.as_slice()),
            ("wk", sparse_attn.wk.as_slice()),
            ("wv", sparse_attn.wv.as_slice()),
            ("wo", sparse_attn.wo.as_slice()),
            ("b2", sparse_b2.as_slice()),
            ("step", &step_value),
            ("s_table_m", &sparse_table_state.m),
            ("s_table_v", &sparse_table_state.v),
            ("s_wq_m", &sparse_wq_state.m),
            ("s_wq_v", &sparse_wq_state.v),
            ("s_wk_m", &sparse_wk_state.m),
            ("s_wk_v", &sparse_wk_state.v),
            ("s_wv_m", &sparse_wv_state.m),
            ("s_wv_v", &sparse_wv_state.v),
            ("s_wo_m", &sparse_wo_state.m),
            ("s_wo_v", &sparse_wo_state.v),
            ("s_b2_m", &sparse_b2_state.m),
            ("s_b2_v", &sparse_b2_state.v),
        ];
        ...
        for (band, m_state, v_state, _w2) in &band_states {
            named.push((
                alloc::format!("s_w1_band{band}_m").leak(),
                m_state.m.as_slice(),
            ));
            named.push((
                alloc::format!("s_w1_band{band}_v").leak(),
                m_state.v.as_slice(),
            ));
            named.push((
                alloc::format!("s_b1_band{band}_m").leak(),
                v_state.m.as_slice(),
            ));
            named.push((
                alloc::format!("s_b1_band{band}_v").leak(),
                v_state.v.as_slice(),
            ));
        }
        for (band, _m, _v, w2_state) in &band_states {
            named.push((
                alloc::format!("s_w2_band{band}_m").leak(),
                w2_state.m.as_slice(),
            ));
            named.push((
                alloc::format!("s_w2_band{band}_v").leak(),
                w2_state.v.as_slice(),
            ));
        }
        ...
        let mut outputs: alloc::vec::Vec<NodeId> = alloc::vec![
            sparse_loss,
            sparse_table_nodes.new_param,
            sparse_table_nodes.new_m,
            sparse_table_nodes.new_v,
            sparse_wq_nodes.new_param,
            sparse_wq_nodes.new_m,
            sparse_wq_nodes.new_v,
            sparse_wk_nodes.new_param,
            sparse_wk_nodes.new_m,
            sparse_wk_nodes.new_v,
            sparse_wv_nodes.new_param,
            sparse_wv_nodes.new_m,
            sparse_wv_nodes.new_v,
            sparse_wo_nodes.new_param,
            sparse_wo_nodes.new_m,
            sparse_wo_nodes.new_v,
            sparse_b2_nodes.new_param,
            sparse_b2_nodes.new_m,
            sparse_b2_nodes.new_v,
        ];
        ...
        let evaluated = evaluate_named(&sparse_train_program, &[], &named, &outputs)
            .expect("sparse training-step program lowers and evaluates");
        sparse_loss_curve.push(evaluated.get(sparse_loss).expect("loss requested").0[0]);

        sparse_attn.table = evaluated
            .get(sparse_table_nodes.new_param)
            .expect("requested")
            .0
            .to_vec();
        sparse_table_state.m = evaluated
            .get(sparse_table_nodes.new_m)
            .expect("requested")
            .0
            .to_vec();
        sparse_table_state.v = evaluated
            .get(sparse_table_nodes.new_v)
            .expect("requested")
            .0
            .to_vec();
        sparse_attn.wq = evaluated
            .get(sparse_wq_nodes.new_param)
            .expect("requested")
            .0
            .to_vec();
        sparse_wq_state.m = evaluated
            .get(sparse_wq_nodes.new_m)
            .expect("requested")
            .0
            .to_vec();
        sparse_wq_state.v = evaluated
            .get(sparse_wq_nodes.new_v)
            .expect("requested")
            .0
            .to_vec();
        sparse_attn.wk = evaluated
            .get(sparse_wk_nodes.new_param)
            .expect("requested")
            .0
            .to_vec();
        sparse_wk_state.m = evaluated
            .get(sparse_wk_nodes.new_m)
            .expect("requested")
            .0
            .to_vec();
        sparse_wk_state.v = evaluated
            .get(sparse_wk_nodes.new_v)
            .expect("requested")
            .0
            .to_vec();
        sparse_attn.wv = evaluated
            .get(sparse_wv_nodes.new_param)
            .expect("requested")
            .0
            .to_vec();
        sparse_wv_state.m = evaluated
            .get(sparse_wv_nodes.new_m)
            .expect("requested")
            .0
            .to_vec();
        sparse_wv_state.v = evaluated
            .get(sparse_wv_nodes.new_v)
            .expect("requested")
            .0
            .to_vec();
        sparse_attn.wo = evaluated
            .get(sparse_wo_nodes.new_param)
            .expect("requested")
            .0
            .to_vec();
        sparse_wo_state.m = evaluated
            .get(sparse_wo_nodes.new_m)
            .expect("requested")
            .0
            .to_vec();
        sparse_wo_state.v = evaluated
            .get(sparse_wo_nodes.new_v)
            .expect("requested")
            .0
            .to_vec();
        sparse_b2 = evaluated
            .get(sparse_b2_nodes.new_param)
            .expect("requested")
            .0
            .to_vec();
        sparse_b2_state.m = evaluated
            .get(sparse_b2_nodes.new_m)
            .expect("requested")
            .0
            .to_vec();
        sparse_b2_state.v = evaluated
            .get(sparse_b2_nodes.new_v)
            .expect("requested")
            .0
            .to_vec();

        for ((_band, w1_values, b1_values, w2_values), (_band2, w1_adam, b1_adam, w2_adam)) in
            band_values.iter_mut().zip(sparse_band_nodes.iter())
        {
            *w1_values = evaluated
                .get(w1_adam.new_param)
                .expect("requested")
                .0
                .to_vec();
            *b1_values = evaluated
                .get(b1_adam.new_param)
                .expect("requested")
                .0
                .to_vec();
            *w2_values = evaluated
                .get(w2_adam.new_param)
                .expect("requested")
                .0
                .to_vec();
        }
        for ((_band, m_w1, v_w1, _w2), (_band2, w1_adam, b1_adam, w2_adam)) in
            band_states.iter_mut().zip(sparse_band_nodes.iter())
        {
        ...
        for ((_band, _m, _v, w2_state), (_band2, _w1_adam, _b1_adam, w2_adam)) in
            band_states.iter_mut().zip(sparse_band_nodes.iter())
        {
    ...
    let fine_tune_final = *sparse_loss_curve
        .last()
        .expect("at least one fine-tune step ran");
    std::eprintln!(
    ...
    assert!(
        sparse_loss_curve.iter().all(|value| value.is_finite()),
        "fine-tune loss went non-finite: {sparse_loss_curve:?}"
    );

    let ffn_named_final: alloc::vec::Vec<(alloc::string::String, alloc::vec::Vec<f32>)> =
        band_values
            .iter()
            ...
            .chain(core::iter::once((
                alloc::string::String::from("b2"),
                sparse_b2.clone(),
            )))
            .collect();
    let ffn_named_final_refs: alloc::vec::Vec<(&str, &[f32])> = ffn_named_final
        .iter()
        .map(|(name, values)| (name.as_str(), values.as_slice()))
        .collect();
    let (generated, matches) = greedy_sample_matches(
        &sparse_program,
        sparse_logits,
        "table",
        &sparse_attn,
        &ffn_named_final_refs,
        &corpus,
        6,
    );
