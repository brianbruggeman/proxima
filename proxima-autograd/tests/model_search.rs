...
fn total_macs(program: &[Op]) -> u64 {
    ...
    program
        ...
        .filter_map(|op| match op {
            Op::Reduce(reduce) if matches!(reduce.body, ScalarOp::Add) => {
                Some(shapes.of(reduce.operand).iter().product::<u64>())
            }
            _ => None,
        })
        .sum()
...
fn ffn_block(
    ...
    params.push(ParamSpec::new(
        "w1",
        alloc::vec![d_model as u32, ffn_hidden as u32],
        w1,
    ));
    let b1 = leaf(
        program,
    ...
    params.push(ParamSpec::new(
        "w2",
        alloc::vec![ffn_hidden as u32, d_model as u32],
        w2,
    ));
...
fn output_head(
    ...
    params.push(ParamSpec::new(
        "w_out",
        alloc::vec![d_model as u32, vocab_size as u32],
        w_out,
    ));
    let b_out = leaf(
        program,
        "b_out",
        alloc::vec![Extent::Static(vocab_size as u32)],
    );
    params.push(ParamSpec::new(
        "b_out",
        alloc::vec![vocab_size as u32],
        b_out,
    ));
    let logits_product = elementwise(
        program,
        ScalarOp::Multiply,
        alloc::vec![(residual2, proj(3, &[0, 1])), (w_out, proj(3, &[1, 2]))],
    );
    let logits_raw = reduce_add(program, logits_product, identity(3), proj(3, &[0, 2]));
    elementwise(
        program,
        ScalarOp::Add,
        alloc::vec![(logits_raw, identity(2)), (b_out, proj(2, &[1]))],
    )
}
...
fn cross_entropy_loss(
    ...
    let stabilized = elementwise(
        program,
        ScalarOp::Add,
        alloc::vec![(probabilities, identity(2)), (log_epsilon, broadcast(2))],
    );
    let log_probabilities = elementwise(
        program,
        ScalarOp::Logarithm,
        alloc::vec![(stabilized, identity(2))],
    );
    ...
    let negated = elementwise(
        program,
        ScalarOp::Negate,
        alloc::vec![(per_position, identity(1))],
    );
    ...
    elementwise(
        program,
        ScalarOp::Multiply,
        alloc::vec![(total, identity(0)), (inv_seq_len, broadcast(0))],
    )
}
...
fn build_attention_network(
    ...
    params.push(ParamSpec::new(
        "wq",
        alloc::vec![d_model as u32, n_heads as u32, head_dim as u32],
        wq,
    ));
    ...
    params.push(ParamSpec::new(
        "wo",
        alloc::vec![n_heads as u32, head_dim as u32, d_model as u32],
        wo,
    ));
    ...
    let attn_product = elementwise(
        &mut program,
        ScalarOp::Multiply,
        alloc::vec![(attended, proj(4, &[0, 1, 2])), (wo, proj(4, &[1, 2, 3]))],
    );
...
fn build_conv_network(
    ...
    for tap in 0..taps {
        let ids_tap = int_leaf(
            &mut program,
            &alloc::format!("ids_tap{tap}"),
            seq_len as u32,
        );
    ...
    for (tap, &x_tap) in tapped.iter().enumerate() {
        ...
        params.push(ParamSpec::new(
            &alloc::format!("w_tap{tap}"),
            alloc::vec![d_model as u32, d_model as u32],
            w_tap,
        ));
        let product = elementwise(
            &mut program,
            ScalarOp::Multiply,
            alloc::vec![(x_tap, proj(3, &[0, 2])), (w_tap, proj(3, &[2, 1]))],
        );
        ...
        mixed = Some(match mixed {
            ...
            Some(previous) => elementwise(
                &mut program,
                ScalarOp::Add,
                alloc::vec![(previous, identity(2)), (partial, identity(2))],
            ),
        });
    }
    ...
    let residual1 = elementwise(
        &mut program,
        ScalarOp::Add,
        alloc::vec![(tapped[0], identity(2)), (mixed, identity(2))],
    );
...
fn build_network(
    ...
    match topology {
        TopologyKind::Attention => {
            build_attention_network(seq_len, vocab_size, d_model, ffn_hidden)
        }
        TopologyKind::Conv(taps) => {
            build_conv_network(seq_len, vocab_size, d_model, taps, ffn_hidden)
        }
    }
}
...
fn make_example(
    ...
    let expected = critical_positions
        .iter()
...
fn induction_task() -> (usize, usize, alloc::vec::Vec<Example>, Example) {
    ...
    let training: alloc::vec::Vec<Example> = raws[0..4]
        .iter()
        .map(|raw| make_example(raw, INDUCTION_SEQ_LEN, INDUCTION_VOCAB, critical.clone()))
        .collect();
...
fn ids_bindings(
    ...
    example: &Example,
    pad_symbol: u32,
) -> alloc::vec::Vec<(alloc::string::String, alloc::vec::Vec<f32>)> {
    if network.taps == 0 {
        alloc::vec![(
            "ids".into(),
            example.ids.iter().map(|&id| id as f32).collect()
        )]
    } else {
        (0..network.taps)
            .map(|tap| {
                (
                    alloc::format!("ids_tap{tap}"),
                    shift_ids(&example.ids, tap, pad_symbol),
                )
            })
            .collect()
    }
...
fn train_network(
    ...
    let table_gathered: alloc::vec::Vec<_> = differentiated
        ...
        .collect();
    assert!(
        !table_gathered.is_empty(),
        "table must be gathered by at least one tap"
    );
    assert_eq!(
        table_gathered.len(),
        network.taps.max(1),
        "table's gather-site count must equal the topology's tap count"
    );
    ...
    let dense_adam: alloc::vec::Vec<(ParamSpec, AdamNodes)> = dense_grads
        .into_iter()
        .map(|(param, grad)| {
            let nodes = append_adam(
                &mut program,
                config,
                param.extents.len() as u16,
                &param.extents,
                &param.name,
                param.node,
                grad,
    ...
    let table_param = leaf(
        ...
        "table",
        alloc::vec![
            Extent::Static(network.vocab_size as u32),
            Extent::Static(network.d_model as u32)
        ],
    );
    ...
    let mut weights = Weights {
        ...
        dense: dense_adam
            ...
            .map(|(index, (param, _))| {
                (
                    param.name.clone(),
                    counter_pattern(23 + index * 7, param.count()),
                )
            })
            .collect(),
    };
    ...
    let mut dense_state: alloc::collections::BTreeMap<alloc::string::String, AdamState> =
        dense_adam
            .iter()
            .map(|(param, _)| (param.name.clone(), zero_state(param.count())))
            .collect();
    ...
    for step in 0..steps {
        ...
        for (param, _) in &dense_adam {
            ...
            bindings.push((
                alloc::format!("{}_m", param.name),
                dense_state[&param.name].m.clone(),
            ));
            bindings.push((
                alloc::format!("{}_v", param.name),
                dense_state[&param.name].v.clone(),
            ));
        }
        let refs: alloc::vec::Vec<(&str, &[f32])> = bindings
            .iter()
        ...
        let evaluated = evaluate_named(&program, &[], &refs, &outputs)
        ...
        for contribution in &table_gathered {
            let indices_values = evaluated
                .get(contribution.indices)
                .expect("gather indices requested")
                .0;
            let gathered_values = evaluated
                .get(contribution.values)
                .expect("gather values requested")
                .0;
            let (unique_ids, summed) =
                sparse::dedupe_and_sum_rows(indices_values, gathered_values, network.d_model)
                    .expect("indices and values line up");
            accumulate_scatter(
                &mut combined_table_grad,
                &unique_ids,
                &summed,
                network.d_model,
            );
        ...
        for (param, nodes) in &dense_adam {
            ...
            let state = dense_state
                .get_mut(&param.name)
                .expect("state tracked for every dense param");
            state.m = new_m;
            state.v = new_v;
        }
    }
...
fn forward_logits(
    ...
    bindings.push((
        "onehot".into(),
        alloc::vec![0.0f32; network.seq_len * network.vocab_size],
    ));
    let refs: alloc::vec::Vec<(&str, &[f32])> = bindings
        .iter()
        .map(|(key, value)| (key.as_str(), value.as_slice()))
        .collect();
    let evaluated = evaluate_named(&network.program, &[], &refs, &[network.logits])
...
fn solved(network: &Network, weights: &Weights, example: &Example, pad_symbol: u32) -> bool {
    ...
    example
        ...
        .all(|(&position, &expected)| {
            ...
            std::eprintln!(
                "  critical position {position}: predicted={predicted} expected={expected}"
            );
            predicted == expected
        })
}
...
fn run_candidate(
    ...
    let windowed_average_loss: f32 = result.loss_curve[result.loss_curve.len() - window..]
        .iter()
        .sum::<f32>()
        / window as f32;
    std::eprintln!(
...
#[proxima::test]
#[case::induction_attention_solves_the_copy_task(
    Task::Induction,
    TopologyKind::Attention,
    8,
    12,
...
#[case::induction_small_conv_cannot_reach_the_cue(
    Task::Induction,
    TopologyKind::Conv(3),
    6,
    8,
...
#[case::induction_larger_conv_has_more_params_and_still_cannot_reach_it(
    Task::Induction,
    TopologyKind::Conv(3),
    16,
    24,
...
#[case::bigram_one_tap_conv_cannot_see_the_second_back_token(
    Task::Bigram,
    TopologyKind::Conv(1),
    6,
    16,
...
async fn a_wider_convolution_has_more_parameters_than_attention_and_still_loses() {
    ...
    assert!(
        !did_solve,
        "the wide conv must still fail: its receptive field, not its parameter count, is the constraint"
    );
}
...
async fn rerunning_the_pipeline_reproduces_byte_identical_results() {
    ...
    for (task, topology, d_model, ffn_hidden) in candidates {
        ...
        assert_eq!(
            first.loss_curve, second.loss_curve,
            "task={task:?} topology={topology:?}: loss curve must rerun identically"
        );
        assert_eq!(
            first.param_count, second.param_count,
            "param count must rerun identically"
        );
        assert_eq!(
            first.forward_macs, second.forward_macs,
            "MAC count must rerun identically"
        );
        assert_eq!(
            first_solved, second_solved,
            "the solved verdict must rerun identically"
        );
    }
}
