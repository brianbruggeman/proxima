...
async fn forward_only_anchored_offset_read_evaluates_the_right_slice() {
    ...
    let evaluated = evaluate_named(&program, &[], &[("w", &w_values)], &[sliced])
        .expect("forward-only anchored slice evaluates");
    ...
    assert_eq!(
        values,
        &[2.0, 3.0, 8.0, 9.0],
        "row0 cols[2,4)=[2,3], row1 cols[2,4)=[8,9]"
    );
}
