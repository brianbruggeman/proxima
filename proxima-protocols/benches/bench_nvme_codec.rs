...
fn sqe_encode(criterion: &mut Criterion) {
    ...
    group.bench_function("burst1024/builder", |bencher| {
        ...
        bencher.iter(|| {
            for (index, slot) in ring.as_chunks_mut::<SQE_LEN>().0.iter_mut().enumerate() {
...
fn cqe_decode(criterion: &mut Criterion) {
    ...
    for (index, slot) in ring.as_chunks_mut::<CQE_LEN>().0.iter_mut().enumerate() {
        proxima_protocols::nvme::write_completion(
            slot,
            0,
            0,
    ...
    group.bench_function("burst1024/borrowed_view", |bencher| {
        bencher.iter(|| {
            let mut acc = 0u32;
            for slot in ring.as_chunks::<CQE_LEN>().0 {
